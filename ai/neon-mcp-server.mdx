...
This configures the Neon MCP Server for compatible MCP clients in your workspace (Cursor, VS Code, Claude Code, and others) using API key authentication. See the [neonctl init documentation](/reference/cli-init).
...
- **API key authentication (remote agents):** For remote agents or when OAuth isn't available:
  ...
  ```
- **Manual configuration:** See [Connect MCP clients](/ai/connect-mcp-clients-to-neon) for step-by-step instructions for any editor, including Windsurf, ChatGPT, Zed, and others.
...
<Warning title="Neon MCP Server Security Considerations">
...
<Tip title="One-click install for Cursor">
...
<Tip title="Install in a single click for Cursor users">
...
**Requires:** Node.js >= v18, [Neon API key](/manage/api-keys)
