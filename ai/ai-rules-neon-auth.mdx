---
title: 'AI Rules: Neon Auth'
subtitle: Context rules for AI tools to help implement authentication with Neon Auth
description: >-
  Covers the implementation of authentication rules for AI tools using Neon
  Auth, providing context and guidelines for effective integration.
---

<Note title="AI Rules are in Beta">
AI Rules are currently in beta. We're actively improving them and would love to hear your feedback. Join us on [Discord](https://discord.gg/92vNTzKDGp) to share your experience and suggestions.
</Note>

**Related docs**

<a href="/guides/neon-auth">Neon Auth</a>

**Repository**

<a href="https://github.com/neondatabase-labs/ai-rules">READ ME</a>
  <a href="https://github.com/neondatabase-labs/ai-rules/blob/main/neon-auth.mdc">neon-auth.mdc</a>

{/* MIGRATION_FLAG: AIRule - Parameterized shared content, Mintlify snippets don't support props */}
<AIRule file="neon-auth.mdc" name="Neon Auth" />

## Rules

{/* MIGRATION_FLAG: ExternalCode - No Mintlify equivalent */}
<ExternalCode
  url="https://raw.githubusercontent.com/neondatabase-labs/ai-rules/main/neon-auth.mdc"
  language="md"
/>
