...
description: >-
  How to connect a GUI application or IDE to a Neon database by gathering
...
![Register - Server](/images/docs/connect/pgadmin4.png)
...
![DBeaver connection](/images/docs/connect/dbeaver_connection.png)
...
| [AWS Database Migration Service (DMS)](https://aws.amazon.com/dms/)                                                           | Use [SNI workaround D](/connect/connection-errors#d-specify-the-endpoint-id-in-the-password-field). Use a `$` character as a separator between the `endpoint` option and password. For example: `endpoint=<endpoint_id>$<password>`. Also, you must set **Secure Socket Layer (SSL) mode** to `require`. See [Migrate with AWS DMS](/import/migrate-aws-dms). |
...
| [Google Looker Studio](https://lookerstudio.google.com/)                                                                      | Requires **Enable SSL** and uploading the PEM-encoded ISRG Root X1 public root certificate issued by Let's Encrypt, which you can find here: [isrgrootx1.pem](https://letsencrypt.org/certs/isrgrootx1.pem). See the [Looker Studio guide](/connect/connect-looker-studio) for detailed connection instructions.                                                   |
...
| [Google Colab](https://colab.research.google.com/)                                                                            | See [Use Google Colab with Neon](/ai/ai-google-colab).                                                                                                                                                                                                                                                                                                             |
...
| [Postico](https://eggerapps.at/postico2/)                                                                                     | SNI support since v1.5.21. For older versions, use [SNI workaround B](/connect/connection-errors#b-use-libpq-keyvalue-syntax-in-the-database-field). Postico's [keep-connection-alive mechanism](https://eggerapps.at/postico/docs/v1.2/changelist.html), enabled by default, may prevent your compute from scaling to zero.                                       |
