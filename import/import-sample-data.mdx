...
- A Neon [paid plan](/introduction/plans) if you intend to install a dataset larger than 0.5 GB.
- Instructions for each dataset require that you create a database. You can do so from a client such as `psql` or from the [Neon SQL Editor](/get-started/query-with-neon-sql-editor).
...
<Note>
If you encounter a memory error related to the `maintenance_work_mem` setting, refer to [Parameter settings that differ by compute size](/reference/compatibility#parameter-settings-that-differ-by-compute-size) for how to increase this setting.
</Note>
...
- [Neon CLI — Install and connect](/reference/cli-install)
- [PostgreSQL Downloads](https://www.postgresql.org/download/) for `psql`

If you have multiple Neon projects or branches, we recommend setting your Neon CLI project and branch context so that you don't have to specify them explicitly when running a Neon CLI command. See [Neon CLI commands — set-context](/reference/cli-set-context).
...
2. Load the data using one of the following Neon CLI commands ([projects](/reference/cli-projects), [branches](/reference/cli-branches), or [connection-string](/reference/cli-connection-string)):
