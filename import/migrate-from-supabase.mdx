...
<Note>
You can also replicate data from Supabase for a near-zero downtime migration. See [Replicate data from Supabase](/guides/logical-replication-supabase-to-neon).
</Note>
...
- A Neon project to move the data to.
  ...
  For detailed information on creating a Neon project, see [Create a project](/manage/projects#create-a-project). Make sure to create a project with the same Postgres version as your Supabase deployment.
...
- Review our guide on [Migrating data from Postgres](/import/migrate-from-postgres) for more comprehensive information on using `pg_dump` and `pg_restore`.
...
To illustrate the migration workflow, we use the [LEGO Database](/import/import-sample-data#lego-database). This database contains information about LEGO sets, parts, and themes.
...
<Warning>
Avoid using `pg_dump` over a [pooled connection string](/reference/glossary#pooled-connection-string) (see PgBouncer issues [452](https://github.com/pgbouncer/pgbouncer/issues/452) & [976](https://github.com/pgbouncer/pgbouncer/issues/976) for details). Use an [unpooled connection string](/reference/glossary#unpooled-connection-string) instead.
</Warning>
...
1. Connect to your Neon project using the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or a Postgres client like [psql](/connect/query-with-psql-editor).
...
For more information, see [Create a database](/manage/databases#create-a-database).
...
1. Connect to your Neon database using the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or `psql`.
...
- **Logical replication**
  ...
  For larger databases or scenarios where you need to minimize downtime, you might consider using logical replication. See our guide on [Logical replication](/guides/logical-replication-guide) for more information.

- **CSV export/import**

  For smaller datasets or specific tables, you might consider exporting to CSV from Supabase and then importing to Neon. See [Import data from CSV](/import/import-from-csv) for more details on this method.
...
- [Migrating data to Neon](/import/migrate-from-postgres)
