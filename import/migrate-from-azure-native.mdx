...
description: >-
  Covers the process of transferring projects from Azure-managed organizations
  to Neon-managed organizations, detailing necessary steps and considerations
  for a successful migration before the Azure integration's end of life.
redirect_from:
...
- You can [rename an organization](/manage/orgs-manage#rename-an-organization) at any time.
...
Projects appear in your destination organization immediately after the transfer completes. For more details about project transfers, see [Transfer projects](/manage/orgs-project-transfer).

## Update your organization configuration

After the transfer:

- Re-add any additional admins, members, or project collaborators who need access. See [Manage organization members](/manage/orgs-manage#add-a-user-to-an-organization) for instructions.
- Verify that all projects appear in your Neon-managed organization.
- For [API keys](/manage/api-keys), project-scoped and personal API keys remain the same after transfer. Organization API keys are tied to your Azure-managed organization, so if you use organization API keys, create new keys in your Neon-managed organization and update them in your applications, scripts, and integrations.
...
If you need help, contact Neon Support through the Neon Console or visit the [support documentation](/introduction/support).
