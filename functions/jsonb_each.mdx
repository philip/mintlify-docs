...
description: >-
  Covers the usage of the `jsonb_each()` function in Postgres to expand a
  `JSONB` object into key-value pairs, facilitating data iteration and
  transformations.
---
...
- [json_each](/functions/json_each) - It provides the same functionality as `jsonb_each`, but accepts `JSON` input instead of `JSONB`.
