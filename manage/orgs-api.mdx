...
The key token is only displayed once at creation time. Copy it immediately and store it securely. If lost, you’ll need to revoke the key and create a new one. For detailed instructions, see [Manage API Keys](/manage/api-keys#create-an-organization-api-key).
...
<Note>The member ID (`id`) from this response is needed for operations like updating roles or removing members.</Note>
...
The API supports transferring projects between organizations. For detailed instructions and examples, see [Transfer projects to an organization](/manage/orgs-project-transfer).
