...
1. Connect to your database from an SQL client such as [psql](/connect/query-with-psql-editor), [pgAdmin](https://www.pgadmin.org/), or the [Neon SQL Editor](/get-started/query-with-neon-sql-editor). If you need help connecting, see [Connect from any client](/connect/connect-from-any-app).
...
2. Create a `readonly` role using the following statement.
   ...
   The password should have at least 12 characters with a mix of lowercase, uppercase, number, and symbol characters. For detailed password guidelines, see [Manage roles with SQL](/manage/roles#manage-roles-with-sql).
...
1. Connect to the database **on the parent branch** from an SQL client such as [psql](/connect/query-with-psql-editor), [pgAdmin](https://www.pgadmin.org/), or the [Neon SQL Editor](/get-started/query-with-neon-sql-editor). If you need help connecting, see [Connect from any client](/connect/connect-from-any-app).
...
4. Create a development branch. Name it something like `dev1`. See [Create a branch](/manage/branches#create-a-branch) for instructions.

5. Connect to the database **on the development branch** with an SQL client. Be mindful that a child branch connection string differs from a parent branch connection string. The branches reside on different hosts. If you need help connecting to your branch, see [Connect from any client](/connect/connect-from-any-app).
