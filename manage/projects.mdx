...
In Neon, the project is your main workspace. Within a project, you create branches for different workflows, like environments, features, or previews. Each branch contains its own databases, roles, computes, and replicas. Your [Neon Plan](/introduction/plans) determines how many projects you can create and the resource limits within those projects.

## Default resources

When you add a new project, Neon creates the following resources by default:

- A root branch is created as your project's default branch. In the Console, this branch is named `production`; via API/CLI, it's named `main`. You can create child branches for development, testing, staging, and other purposes. For more information, see [Manage branches](/manage/branches).
- A single primary read-write compute. This is the compute associated with the branch. For more information, see [Manage computes](/manage/computes).
...
![Project Settings page](/images/docs/manage/settings_page.png)
...
- **Instant restore**: Set the [restore window](/introduction/restore-window) to enable instant restore, time travel queries, and branching from past states.
...
![default_compute_settings](/images/docs/manage/default_compute_settings.png)
...
- **Autoscaling:** Specify minimum and maximum compute sizes (from .25 CU to 16 CUs) to automatically scale based on workload. Note: The maximum permitted autoscaling range is 8 CU, meaning the difference between max and min cannot exceed 8 CU (for example, if min = 1 CU, max can be at most 9 CU). For more information, see [Autoscaling](/introduction/autoscaling)
...
- [Instant restore](/introduction/branch-restore) for recovering lost data
- [Time Travel](/guides/time-travel-assist) queries for investigating data issues

For complete details about the restore window feature, including plan limits, how it works, and storage implications, see [Restore window](/introduction/restore-window).
...
3. Select **Restore window**.
   ![Restore window configuration](/images/docs/manage/instant_restore_setting.png)
...
For more information, see [Updates](/manage/updates).
...
For additional information, refer to our [Project collaboration guide](/guides/project-collaboration-guide).
...
The [Neon CLI ip-allow command](/reference/cli-ip-allow) supports IP Allow configuration. For example, the following `add` command adds IP addresses to the allowlist for an existing Neon project. Multiple entries are separated by a space. No delimiter is required.
...
You can define an allowlist with individual IP addresses, IP ranges, or [CIDR notation](/reference/glossary#cidr-notation). A combination of these options is also permitted. Multiple entries, whether they are the same or of different types, must be separated by a comma. Whitespace is ignored.
...
- **Use CIDR notation**: For more advanced control, you can use [CIDR (Classless Inter-Domain Routing) notation](/reference/glossary#cidr-notation). This is a compact way of defining a range of IPs and is useful for larger networks or subnets. Using CIDR notation can be advantageous when managing access to branches with numerous potential users, such as in a large development team or a company-wide network.
...
The Neon CLI provides an `ip-allow` command with `add`, `reset`, and `remove` options that you can use to update your IP Allow configuration. For instructions, refer to [Neon CLI commands — ip-allow](/reference/cli-ip-allow).
...
The [Neon CLI ip-allow command](/reference/cli-ip-allow) supports removing an IP Allow configuration. To do so, specify `--ip-allow reset` without specifying any IP address values:
...
For setup instructions and examples, see the [Data API documentation](/data-api/get-started).
...
After enabling logical replication, the next steps involve creating publications on your replication source database in Neon and configuring subscriptions on the destination system or service. To get started, refer to our [logical replication guides](/guides/logical-replication-guide).
...
<Note>
The deletion recovery period is different from the [restore window](/introduction/restore-window). The restore window enables point-in-time recovery (PITR) for branch data, while the deletion recovery period allows you to recover (undelete) an entire deleted project.
</Note>
...
For more information about the Neon CLI, see [Neon CLI — projects](/reference/cli-projects).
