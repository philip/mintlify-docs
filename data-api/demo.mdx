...
![Notes app UI](/images/docs/data-api/all_notes.png)
...
This enables both the Data API and Neon Auth in one step. For detailed instructions, see [Getting started with Data API](/data-api/get-started).
...
The demo app uses [Drizzle ORM](/guides/rls-drizzle) to define RLS policies, which we highly recommend as a simpler, more maintainable way of writing RLS policies. Here's how the notes table defines its policies:
...
<Info title="About auth.user_id()">
Neon's RLS policies use the <code>auth.user_id()</code> function, which extracts the user's ID from the JWT (JSON Web Token) provided by your authentication provider. In this demo, <a href="/auth/overview">Neon Auth</a> issues the JWTs, and Neon's Data API passes them to Postgres, so RLS can enforce per-user access.

For more details on RLS with Data API, see our [Row-Level Security with Neon guide](/guides/row-level-security).
</Info>
...
![New note UI](/images/docs/data-api/new_note.png)
...
![Notes list UI](/images/docs/data-api/all_notes.png)
...
You can chain methods like `.from()`, `.update()`, and `.eq()` to build queries. For more complex queries, refer to the [Neon TypeScript SDK documentation](/reference/javascript-sdk#insert).

Here's how a note looks after you update its title:

![Note title updated UI](/images/docs/data-api/renamed_note.png)
...
![notes app with delete](/images/docs/data-api/app_with_delete.png)
...
- [Getting started with Data API](/data-api/get-started)
- [Generate TypeScript types](/data-api/generate-types)
- [SQL to REST Converter](/data-api/sql-to-rest)
- [Neon Auth documentation](/auth/overview)
- [Neon Auth & Data API TypeScript SDKs](/reference/javascript-sdk)
