...
- Neon Data API does not currently support projects with [IP Allow](/manage/projects#configure-ip-allow) or [Private Networking](/guides/neon-private-networking) enabled.
...
![Data API page with enable button](/images/docs/data-api/data_api_sidebar.png)

### 2. Configure Neon Auth (optional)

The **Use Neon Auth** checkbox allows you to enable [Neon Auth](/auth/overview) as your authentication provider for the Data API. When enabled, Neon Auth manages sign-up, login, and account access, issuing the JWTs required for API requests.

If you prefer to use a different authentication provider (such as Auth0, Clerk, or Firebase Auth), leave this checkbox unchecked and configure your provider later. See [Custom authentication providers](/data-api/custom-authentication-providers) for details.

<Note title="Authentication required">
...
Enable this checkbox unless you need to manage permissions manually. If you leave it unchecked, see [Access control](/data-api/access-control) for details on granting permissions yourself.

### 4. Click Enable Data API

Click **Enable Data API** to activate the Data API. Once enabled, you'll see the Data API page.

![Data API enabled view](/images/docs/data-api/data-api-enabled.png)
...
For advanced configuration options like custom authentication providers, exposed schemas, and CORS settings, see the **Settings** tab or refer to [Managing the Data API](/data-api/manage).
...
- **SQL**: Write SQL directly in the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or manage migrations manually. See our [PostgreSQL RLS tutorial](/postgresql/postgresql-administration/postgresql-row-level-security) for more on RLS fundamentals.
...
For more on Drizzle RLS, see our [Drizzle RLS guide](/guides/rls-drizzle).
...
<Info title="What is auth.user_id() and authUid()?">
...
![Data API refresh schema cache button](/images/docs/data-api/data_api_schema_refresh.png)
...
<Tab title="Other authentication provider">
...
> For detailed guidance on performing `INSERT`, `UPDATE`, `DELETE`, and advanced queries (filters, joins, stored procedures, etc.) in either case, refer to the [Neon Javascript SDK documentation](/reference/javascript-sdk#insert).
...
- **Neon Auth (programmatic)**: Retrieve the token using `client.auth.getSession()` from the `@neondatabase/neon-js` library. See [Get current session](/reference/javascript-sdk#auth-getsession) for details.
- **Other providers**: Retrieve the token from your auth provider's SDK (for example, `getAccessToken()` in Auth0, `getToken()` in Clerk).

**About the `sub` claim:**

For RLS policies to work correctly, the JWT token must include a `sub` (subject) claim, which contains the user's unique identifier. The Data API uses this claim to enforce [Row-Level Security](/guides/neon-rls) policies via the `auth.user_id()` function. Most authentication providers include this claim by default.
...
The Data API supports full CRUD operations and advanced queries. Here's a quick reference of the most common methods available in the [Neon TypeScript SDK](/reference/javascript-sdk):

### CRUD operations

| Operation  | Method      | Example                                                         | SDK Reference                                   |
| ---------- | ----------- | --------------------------------------------------------------- | ----------------------------------------------- |
| **Select** | `.select()` | `client.from('posts').select('*')`                              | [select](/reference/javascript-sdk#select) |
| **Insert** | `.insert()` | `client.from('posts').insert({ title: 'New post' })`            | [insert](/reference/javascript-sdk#insert) |
| **Update** | `.update()` | `client.from('posts').update({ title: 'Updated' }).eq('id', 1)` | [update](/reference/javascript-sdk#update) |
| **Delete** | `.delete()` | `client.from('posts').delete().eq('id', 1)`                     | [delete](/reference/javascript-sdk#delete) |
| **RPC**    | `.rpc()`    | `client.rpc('function_name', { param: 'value' })`               | [rpc](/reference/javascript-sdk#rpc)       |
...
For the complete list of methods and detailed examples, see the [Neon Auth & Data API TypeScript SDKs](/reference/javascript-sdk).

## Next steps

- [Build a note-taking app](/data-api/demo): Hands-on tutorial with Data API queries
- [Neon Auth & Data API TypeScript SDKs](/reference/javascript-sdk): All database methods: select, insert, update, delete, filters, and more
- [Generate TypeScript types](/data-api/generate-types): Get autocomplete for table names and columns
- [SQL to REST Converter](/data-api/sql-to-rest): Convert SQL queries to API calls
- [Row-Level Security with Neon](/guides/row-level-security): Secure your data at the database level
