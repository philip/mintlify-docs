...
- The `GRANT` statements would be similar to the grants for the `authenticated` role. See [Configure schema access](/data-api/get-started#3-configure-schema-access) for an example.
...
Run these commands in the [SQL Editor](/get-started/query-with-neon-sql-editor) to ensure your API users can access your tables:
...
<Info title="Permission denied errors?">
...
<Warning title="RLS disabled means no filtering">
