...
![Auth Configuration > Organizations in the Neon Console](/images/docs/auth/console-auth-organizations-config.png)
...
<Note title="Invitation Emails">
Invitation emails are not sent during the Beta phase. They will be supported in a future release. In the meantime, users can accept invitations using the [invitation ID](/auth/guides/plugins/organization#accept-invitation) or by viewing them in their [invitation list](/auth/guides/plugins/organization#list-user-invitations).
</Note>
...
Check the [Neon Auth roadmap](/auth/roadmap) for updates on these features.
