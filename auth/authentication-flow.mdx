...
<Note>
Anyone can sign up for your application by default. Support for restricted signups is coming soon. Until then, consider adding a verification step by enabling [email verification](/auth/guides/email-verification) via verification link or verification code.
</Note>
...
<Note>
By default, anyone can sign up for your application. To add an additional verification layer, enable email verification (see [Email Verification](/auth/guides/email-verification)). Built-in signup restrictions are coming soon.
</Note>
...
When you enable the [Data API](/data-api/get-started), JWT tokens from Neon Auth are validated automatically. The user ID is available via the `auth.uid()` function, enabling Row-Level Security policies to grant data access based on the authenticated user.
...
<CardGroup>
  ...
  <Card title="Branching Authentication" icon="split-branch" href="/auth/branching-authentication">
    How auth works with database branches
  </Card>

  <Card title="Row Level Security" icon="check" href="/guides/row-level-security">
