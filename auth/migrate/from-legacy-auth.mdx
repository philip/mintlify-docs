...
<Warning title="Legacy Neon Auth (Stack Auth) is no longer accepting new users">
...
export function Header() {
  return {/* MIGRATION_FLAG: UserButton - No Mintlify equivalent */}
<UserButton />;
}
```
...
function App() {
  return {/* MIGRATION_FLAG: NeonAuthUIProvider - No Mintlify equivalent */}
<NeonAuthUIProvider authClient={authClient}>{/* Your app */}</NeonAuthUIProvider>;
}
```
