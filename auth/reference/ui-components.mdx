...
**Note:** Google OAuth works with shared credentials for development. GitHub OAuth requires custom credentials. The `social.providers` prop controls which provider buttons are displayed in the UI. For production, configure your own OAuth credentials in the Neon Console (Settings → Auth). See the [OAuth setup guide](/auth/guides/setup-oauth) for details.
...
function App() {
  return {/* MIGRATION_FLAG: AuthView - No Mintlify equivalent */}
...
- See [React with Neon Auth UI](/auth/quick-start/react-router-components) for a complete example
- Check the [Neon Auth & Data API TypeScript SDKs](/reference/javascript-sdk) for programmatic auth methods
