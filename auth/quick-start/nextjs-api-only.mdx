...
<Note>
Upgrading from Neon Auth SDK v0.1? See the [migration guide](/auth/migrate/from-auth-v0.1) for step-by-step instructions.
</Note>

This guide shows you how to integrate Neon Auth into a [Next.js](https://nextjs.org) (App Router) project using SDK methods directly. To use our pre-built UI components instead, see the [UI components guide](/auth/quick-start/nextjs).
...
- [authClient.signUp.email()](/reference/javascript-sdk#auth-signup) / `auth.signUp.email()` - Create a new user account
- [authClient.signIn.email()](/reference/javascript-sdk#auth-signinwithpassword) / `auth.signIn.email()` - Sign in with email and password
- [authClient.signOut()](/reference/javascript-sdk#auth-signout) / `auth.signOut()` - Sign out the current user
- [authClient.getSession()](/reference/javascript-sdk#auth-getsession) / `auth.getSession()` - Get the current session
...
- [Branching authentication](/auth/branching-authentication)
