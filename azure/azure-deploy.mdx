...
description: >-
  Step-by-step guide for deploying Neon as a native service on Azure, including
  prerequisites and instructions for creating projects in Azure regions without
  native integration.
isDraft: false
---
...
<a href="/manage/azure">Neon on Azure</a>
  <a href="/azure/azure-develop">Developing with Neon on Azure</a>
...
1. Use the search bar at the top of the [Azure portal](https://portal.azure.com/) to find the **Neon Serverless Postgres** offering.
   ...
   ![Search for Neon in the Azure Portal](/images/docs/introduction/azure_search_neon.png)
...
1. On the **Create a Neon Serverless Postgres Resource** page, enter values for the properties described below.
   ...
   | **Region**                 | Select a region to deploy your Azure resource. This is the region for your Azure resource, not for your Neon projects and data. You will select from [Azure-supported regions](/introduction/regions#azure-regions) when you create the Neon project in a later step. For example, you can create a Neon resource in the (US) West US 3 region and create a Neon project (Europe) Germany West Central. |
   | **Neon Organization name** | Provide a name for your [Neon Organization](/reference/glossary#organization), such as a team name or company name. The name you specify will be your [Organization](/reference/glossary#organization) name in the Neon Console. Your Neon projects will reside in this named organization.                                                                                                        |
   ...
   ![Create a resource group](/images/docs/introduction/azure_create_resource_group.png)
...
1. On the **Project** page, enter a name for your Neon project, select a Postgres version, specify a name for your database, and choose a region. We recommend selecting the region closest to your application.
   ...
   ![Create project in Azure](/images/docs/introduction/azure_create_project.png)

      <Note>
        A Neon organization created via the Azure portal supports creating Neon projects in [Azure regions](/introduction/regions#azure-regions) only. Neon's AWS regions are not supported with Neon on Azure.
      </Note>
...
1. When your deployment is complete, click the **Go to resource** button under **Next steps** to view your new Neon resource.
   ...
   ![Go to resource button](/images/docs/introduction/azure_go_to_resource.png)

1. Select the **Go to Neon** link under **Getting started** to open the Neon Console.

   ![Azure get started](/images/docs/introduction/azure_get_started.png)
   ...
   ![Project page in Neon](/images/docs/introduction/azure_neon_project_page.png)

1. From here, follow the [Neon Getting Started](/get-started/signing-up) guide to begin working with your Neon project and get familiar with the platform.
...
To create a Neon project from the console, follow the [Create project](/manage/projects#create-a-project) steps. Select **Azure** as the **Cloud Service Provider**, and choose one of the available [Azure regions](/introduction/regions).

</Tab>

<Tab title="CLI">

To create a Neon project using the Neon CLI, use the [neon projects create](/reference/cli-projects#create) command:

```bash
neon projects create --name mynewproject --region-id azure-eastus2
```

For Azure `region-id` values, see [Regions](/introduction/regions).
</Tab>
...
For Azure `region_id` values, see [Regions](/introduction/regions).
