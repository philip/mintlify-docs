...
Neon manages **minor** Postgres version upgrades for you, as per the [Neon Postgres Version Support Policy](/postgresql/postgres-version-policy). Typically, no user action is required for **minor** version upgrades. Neon deploys minor versions soon after they become available. However, upgrading to a new major Postgres version is a manual task that must be performed by you.

Each Neon project is tied to a specific Postgres major version, which you selected when creating your Neon project.

You can check your Neon project's Postgres version in the **Settings** widget on **Project Dashboard** or by running the following query from the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or any SQL client connection to your database:
...
Start by creating a new Neon project with the desired Postgres version. For instructions, see [creating a new Neon project](/manage/projects#create-a-project).
...
- [Compute size](/manage/computes#edit-a-compute)
- [Autoscaling](/guides/autoscaling-guide)
- [Scale to Zero](/guides/scale-to-zero-guide)
- [Protected branches](/guides/protected-branches)
- [IP Allow](/introduction/ip-allow)
...
- **Dump and restore**
  ...
  - [Migrate data from one Neon project to another by piping data from pg_dump to pg_restore](/import/migrate-from-neon)
...
You can find the connection details for your database by clicking the **Connect** button on your **Project Dashboard**. This opens the **Connect to your database** modal. See [Connect from any application](/connect/connect-from-any-app).
