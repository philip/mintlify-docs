...
<Warning title="Key details about using pg_cron with Neon">
Please note that `pg_cron` jobs will only run when your compute is active. We therefore recommend only using `pg_cron` on computes that run 24/7 or where you have disabled [scale to zero](/introduction/scale-to-zero).
</Warning>
...
The `cron.database_name` parameter is passed to your Postgres instance through the `pg_settings` option in the endpoint settings object. The following `Update endpoint` API example shows where to specify your Neon `project_id`, `endpoint_id`, [Neon API key](/manage/api-keys), and database name.
...
<Note>
The [Restart compute endpoint](https://api-docs.neon.tech/reference/restartprojectendpoint) API only works on an active compute. If your compute is idle, you can start it by running a query to wake it up or running the [Start compute endpoint](https://api-docs.neon.tech/reference/startprojectendpoint) API. For more information and other compute restart options, see [Restart a compute](/manage/computes#restart-a-compute).
</Note>

You can then install the `pg_cron` extension by running the following `CREATE EXTENSION` statement in the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or from a client such as [psql](/connect/query-with-psql-editor) that is connected to your Neon database.
...
<Warning title="Function not supported in Neon">
