...
You can enable the `pgvector` extension by running the following `CREATE EXTENSION` statement in the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or from a client such as [psql](/connect/query-with-psql-editor) that is connected to Neon.
...
To check the latest supported `pgvector` version on Neon, visit our [Postgres extensions page](/extensions/pg-extensions). You can install one version back from that version.
...
- `maintenance_work_mem`
  ...
  In Postgres, the `maintenance_work_mem` setting determines the maximum memory allocation for tasks such as `CREATE INDEX`. The default `maintenance_work_mem` value in Neon is set according to your Neon [compute size](/manage/computes#how-to-size-your-compute).
...
- `maintenance_work_mem`
  ...
  In Postgres, the `maintenance_work_mem` setting determines the maximum memory allocation for tasks such as `CREATE INDEX`. The default `maintenance_work_mem` value in Neon is set according to your Neon [compute size](/manage/computes#how-to-size-your-compute). For a table that shows the `maintenance_work_mem` setting by compute size, see [Parameter settings that differ by compute size](/reference/compatibility#parameter-settings-that-differ-by-compute-size).
