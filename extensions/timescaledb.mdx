...
description: >-
  Covers the setup and usage of the `timescaledb` extension in Neon for
...
**Version availability:** For version availability, refer to our [Postgres extensions](/extensions/pg-extensions) page, which includes a table of supported extension versions by Postgres version.
...
For information about using the Neon SQL Editor, see [Query with Neon's SQL Editor](/get-started/query-with-neon-sql-editor). For information about using the `psql` client with Neon, see [Connect with psql](/connect/query-with-psql-editor).
...
You can use the [sample weather dataset from TimescaleDB](https://assets.timescale.com/docs/downloads/weather_small.tar.gz) and load it into your Neon database using [psql](/connect/query-with-psql-editor).
...
<Note>
Please be aware that Neon's [Scale to Zero](/guides/scale-to-zero-guide) feature may affect the running of scheduled jobs. It may be necessary to start the compute before running a job.
</Note>
