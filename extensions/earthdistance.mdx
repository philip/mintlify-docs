...
description: >-
  Covers the setup and usage of the `earthdistance` extension in Postgres for
  calculating great-circle distances between geographic points, essential for
...
<Warning title="Accuracy and assumptions">
The `earthdistance` extension primarily assumes a spherical Earth model for its calculations, which provides good approximations for many use cases. It relies on the [`cube`](/extensions/cube) extension for some of its underlying operations.

You may consider using the [`postgis` extension](/extensions/postgis) if accurate geospatial calculations are critical for your application.
</Warning>
...
To use `earthdistance`, you first need to enable it and its dependency, the [`cube` extension](/extensions/cube). You can do this by running the following `CREATE EXTENSION` statements in the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or from a client like [psql](/connect/query-with-psql-editor):
...
- [Cube extension](/extensions/cube)
