...
<Note title="pg_search on Neon">

`pg_search` is currently only available on Neon projects created in an [AWS region](/introduction/regions#aws-regions). It is not yet supported on Neon projects created in Azure regions.
...
<Tab title="Postgres 17">

Install the `pg_search` extension by running the following `CREATE EXTENSION` statement in the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or from a client such as [psql](/connect/query-with-psql-editor) that is connected to your Neon database.
...
First, connect to your Neon database using the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or a client like [psql](/connect/query-with-psql-editor). Once connected, execute the following SQL command:
...
<Note title="Key Field Selection">
...
- **`maintenance_work_mem`**: : Sets the maximum amount of memory used for maintenance operations such as `CREATE INDEX`. Increasing this setting can speed up index builds by improving Write-Ahead Log (WAL) performance. For example, on a 100-million-row table, allocating multiple GBs can reduce index build time from hours to minutes.
  ...
  In Neon, `maintenance_work_mem` is set based on your compute size. You can increase it for the current session. Do not exceed 50–60% of your compute's available RAM. See [Neon parameter settings by compute size](/reference/compatibility#parameter-settings-that-differ-by-compute-size).
...
- **`max_parallel_workers_per_gather`**: Limits parallel workers per query. The default in Neon is `2`, but you can adjust. The total number of parallel workers should not exceed your Neon compute's CU count. See [Neon parameter settings by compute size](/reference/compatibility#parameter-settings-that-differ-by-compute-size).
  `sql
...
Both `shared_buffers` and the LFC size depend on your compute size. For details, see [How to size your compute](/manage/computes#how-to-size-your-compute).
...
For additional details, see [Running pg_prewarm on indexes](/extensions/pg_prewarm#running-pgprewarm-on-indexes).
