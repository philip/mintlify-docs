---
title: Postgres extension explorer
description: >-
  Covers the browsing of supported Postgres extensions categorized by
  functionality, including installation and update instructions for various
  extensions related to AI, analytics, and auditing.
redirect_from:
  - /docs/extensions/extensions-intro
---

Browse supported Postgres extensions by category.

[List view](/extensions/pg-extensions) | [Install an extension](/extensions/pg-extensions#install-an-extension) | [Update an extension](/extensions/pg-extensions#update-an-extension-version) | [Request an extension](/extensions/pg-extensions#request-an-extension)

## AI / Machine Learning

<CardGroup>


  <Card title="pg_tiktoken" icon="/images/icons/sparkle.svg" href="/extensions/pg_tiktoken">
    Tokenize data in Postgres using the OpenAI tiktoken library
  </Card>

  <Card title="pgrag" icon="/images/icons/sparkle.svg" href="/extensions/pgrag">
    Create end-to-end Retrieval-Augmented Generation (RAG) pipelines
  </Card>

  <Card title="pgvector" icon="/images/icons/sparkle.svg" href="/extensions/pgvector">
    Store vector embeddings and perform vector similarity search in Postgres
  </Card>


</CardGroup>

## Analytics

<CardGroup>


  <Card title="pg_mooncake" icon="/images/icons/a-chart.svg" href="/extensions/pg_mooncake">
    Adds columnstore tables and DuckDB execution for fast analytics in Postgres.
  </Card>

  <Card title="hll" icon="/images/icons/a-chart.svg" href="https://github.com/citusdata/postgresql-hll">
    Implements a HyperLogLog data structure as a native data type for efficient and tunable distinct value counting
  </Card>

  <Card title="timescaledb" icon="/images/icons/a-chart.svg" href="/extensions/timescaledb">
    Enables Postgres as a time-series database for efficient storage and retrieval of time-series data
  </Card>


</CardGroup>

## Auditing / Logging

<CardGroup>


  <Card title="insert_username" icon="/images/icons/check.svg" href="https://www.postgresql.org/docs/current/contrib-spi.html">
    Implements a trigger that stores the current user's name into a text field, useful for tracking who modified a particular row within a table
  </Card>

  <Card title="moddatetime" icon="/images/icons/check.svg" href="https://www.postgresql.org/docs/current/contrib-spi.html">
    Implements a trigger that automatically updates a timestamp column to the current timestamp whenever a row is modified
  </Card>

  <Card title="pgrowlocks" icon="/images/icons/check.svg" href="/extensions/pgrowlocks">
    Provides a function that shows row locking information for a specified table, useful in concurrency and deadlock debugging
  </Card>

  <Card title="tcn" icon="/images/icons/check.svg" href="https://www.postgresql.org/docs/16/tcn.html">
    Provides a trigger function to notify listeners of changes to a table, allowing applications to respond to changes in the database
  </Card>


</CardGroup>

## Data / Transformations

<CardGroup>

  <Card title="address_standardizer" icon="/images/icons/data.svg" href="https://postgis.net/docs/Extras.html#Address_Standardizer">
    A single-line address parser that takes an input address and normalizes it based on a set of rules
  </Card>

  <Card title="address_standardizer_data_us" icon="/images/icons/data.svg" href="https://postgis.net/docs/Extras.html#Address_Standardizer_Tables">
    Provides data for standardizing US addresses, for use with the address_standardizer extension
  </Card>

  <Card title="anon" icon="/images/icons/data.svg" href="/extensions/postgresql-anonymizer">
    Provides data masking and anonymization capabilities for protecting personally identifiable information (PII) and sensitive data in Postgres databases
  </Card>

  <Card title="citext" icon="/images/icons/data.svg" href="/extensions/citext">
    Provides a case-insensitive character string type that internally calls lower when comparing values in Postgres
  </Card>

  <Card title="cube" icon="/images/icons/data.svg" href="/extensions/cube">
    Implements the cube data type for representing multidimensional cubes in Postgres
  </Card>

  <Card title="earthdistance" icon="/images/icons/data.svg" href="/extensions/earthdistance">
    Provides cube-based and point-based approaches to calculating great circle distances on the surface of the Earth
  </Card>

  <Card title="hstore" icon="/images/icons/data.svg" href="/extensions/hstore">
    Implements an hstore data type for storing and manipulating sets of key-value pairs within a single Postgres value
  </Card>

  <Card title="intagg" icon="/images/icons/data.svg" href="https://www.postgresql.org/docs/16/intagg.html">
    Provides an integer aggregator and enumerator for Postgres
  </Card>

  <Card title="intarray" icon="/images/icons/data.svg" href="/extensions/intarray">
    Offers functions and operators for manipulating and searching arrays of integers within Postgres
  </Card>

  <Card title="isn" icon="/images/icons/data.svg" href="https://www.postgresql.org/docs/16/isn.html">
    Implements data types for international product numbering standards: EAN13, UPC, ISBN (books), ISMN (music), and ISSN (serials)
  </Card>

  <Card title="ltree" icon="/images/icons/data.svg" href="/extensions/ltree">
    Provides data types for representing labels of data stored in a hierarchical tree-like structure and facilities for searching through label trees
  </Card>

  <Card title="pg_graphql" icon="/images/icons/data.svg" href="/extensions/pg_graphql">
    Adds GraphQL support to Postgres, allowing you to query your database via GraphQL
  </Card>

  <Card title="pg_hashids" icon="/images/icons/data.svg" href="https://github.com/iCyberon/pg_hashids">
    Enables the generation of short, unique hash ids from integers, useful for obfuscating internal ids
  </Card>

  <Card title="pg_jsonschema" icon="/images/icons/data.svg" href="https://github.com/supabase/pg_jsonschema">
    Provides support for JSON schema validation on json and jsonb data types
  </Card>

  <Card title="pg_uuidv7" icon="/images/icons/data.svg" href="/extensions/pg_uuidv7">
    Enables creating valid UUID Version 7 values in Postgres, enabling globally unique identifiers with temporal ordering
  </Card>

  <Card title="pgx_ulid" icon="/images/icons/data.svg" href="https://github.com/pksunkara/pgx_ulid">
    A full-featured extension for generating and working with ULID (Universally Unique Lexicographically Sortable Identifiers)
  </Card>

  <Card title="seg" icon="/images/icons/data.svg" href="https://www.postgresql.org/docs/16/seg.html">
    Implements the seg data type for storage and manipulation of line segments or floating-point ranges, useful for geometric and scientific applications
  </Card>

  <Card title="semver" icon="/images/icons/data.svg" href="https://pgxn.org/dist/semver">
    A Postgres data type for the Semantic Version format with support for btree and hash indexing
  </Card>

  <Card title="tablefunc" icon="/images/icons/data.svg" href="/extensions/tablefunc">
    Contains functions that return tables (multiple rows), including crosstab, which can pivot row data into columns dynamically
  </Card>

  <Card title="unaccent" icon="/images/icons/data.svg" href="/extensions/unaccent">
    A text search dictionary that removes accents from characters, simplifying text search in Postgres
  </Card>

  <Card title="unit" icon="/images/icons/data.svg" href="https://github.com/df7cb/postgresql-unit">
    Implements a data type for SI units, plus byte, for storage, manipulation, and calculation of scientific units
  </Card>

  <Card title="uuid-ossp" icon="/images/icons/data.svg" href="/extensions/uuid-ossp">
    Provides functions to generate universally unique identifiers (UUIDs) in Postgres, supporting various UUID standards
  </Card>

  <Card title="wal2json" icon="/images/icons/data.svg" href="/extensions/wal2json">
    A Postgres logical decoding plugin that converts Write-Ahead Log (WAL) changes into JSON objects
  </Card>

  <Card title="xml2" icon="/images/icons/data.svg" href="/extensions/xml2">
    Enables XPath queries and XSLT functionality directly within Postgres, enabling XML data processing
  </Card>


</CardGroup>

## Debugging

<CardGroup>


  <Card title="moddatetime" icon="/images/icons/bug.svg" href="https://www.postgresql.org/docs/current/contrib-spi.html">
    Automatically updates a timestamp column to the current timestamp whenever a row is modified in Postgres
  </Card>

  <Card title="pgrowlocks" icon="/images/icons/bug.svg" href="/extensions/pgrowlocks">
    Provides a function that shows row locking information for a specified table, which can aid in concurrency and deadlock debugging
  </Card>

  <Card title="pgTap" icon="/images/icons/bug.svg" href="https://pgtap.org/documentation.html">
    A unit testing framework for Postgres, enabling sophisticated testing of database queries and functions
  </Card>

  <Card title="plpgsql_check" icon="/images/icons/bug.svg" href="https://pgxn.org/dist/plpgsql_check/">
    Provides a linter and debugger for PL/pgSQL code, helping identify errors and optimize PL/pgSQL functions
  </Card>


</CardGroup>

## Geospatial

<CardGroup>


  <Card title="cube" icon="/images/icons/globe.svg" href="/extensions/cube">
    Implements a data type for representing multidimensional cubes in Postgres
  </Card>

  <Card title="earthdistance" icon="/images/icons/globe.svg" href="/extensions/earthdistance">
    Provides cube-based and point-based approaches to calculating great circle distances on the surface of the Earth
  </Card>

  <Card title="h3" icon="/images/icons/globe.svg" href="/extensions/postgis-related-extensions#h3-and-h3-postgis">
    Integrates Uber's H3 geospatial indexing system that combines the benefits of a hexagonal grid with S2's hierarchical subdivisions
  </Card>

  <Card title="h3_postgis" icon="/images/icons/globe.svg" href="/extensions/postgis-related-extensions#h3-and-h3-postgis">
    A PostGIS extension for H3, enabling advanced spatial analysis and indexing
  </Card>

  <Card title="pgrouting" icon="/images/icons/globe.svg" href="/extensions/postgis-related-extensions#pgrouting">
    Extends PostGIS/Postgres databases, providing geospatial routing and other network analysis functionality
  </Card>

  <Card title="postgis" icon="/images/icons/globe.svg" href="/extensions/postgis">
    Extends Postgres to allow GIS (Geographic Information Systems) objects to be stored in the database, enabling spatial queries directly in SQL
  </Card>

  <Card title="postgis_raster" icon="/images/icons/globe.svg" href="https://postgis.net/docs/RT_reference.html">
    Adds support for raster data to PostGIS, enabling advanced geospatial analysis on raster images
  </Card>

  <Card title="postgis_sfcgal" icon="/images/icons/globe.svg" href="/extensions/postgis-related-extensions#postgis-sfcgal">
    Provides support for advanced 3D geometries in PostGIS, based on the SFCGAL library
  </Card>

  <Card title="postgis_tiger_geocoder" icon="/images/icons/globe.svg" href="/extensions/postgis-related-extensions#postgis-tiger-geocoder">
    Enables geocoding and reverse geocoding capabilities in PostGIS using TIGER/Line data
  </Card>

  <Card title="postgis_topology" icon="/images/icons/globe.svg" href="https://www.postgis.net/docs/Topology.html">
    Extends PostGIS with support for topological data types and functions, facilitating the analysis of spatial relationships
  </Card>


</CardGroup>

## Index / Table optimization

<CardGroup>


  <Card title="bloom" icon="table" href="https://www.postgresql.org/docs/16/bloom.html">
    Provides an index access method for Postgres based on Bloom filters
  </Card>

  <Card title="btree_gin" icon="table" href="/extensions/btree_gin">
    Provides GIN operator classes that implement B-tree equivalent behavior
  </Card>

  <Card title="btree_gist" icon="table" href="/extensions/btree_gist">
    Provides GiST index operator classes that implement B-tree equivalent behavior
  </Card>

  <Card title="ip4r" icon="table" href="https://github.com/RhodiumToad/ip4r">
    Provides a range index type and functions for efficiently storing and querying IPv4 and IPv6 ranges and addresses in Postgres
  </Card>

  <Card title="pg_ivm" icon="table" href="https://github.com/sraoss/pg_ivm">
    Provides an Incremental View Maintenance (IVM) feature for Postgres
  </Card>

  <Card title="pg_partman" icon="table" href="https://github.com/pgpartman/pg_partman">
    A partition manager extension that enables creating and managing time-based and number-based table partition sets in Postgres
  </Card>

  <Card title="pg_prewarm" icon="table" href="/extensions/pg_prewarm">
    Allows manual preloading of relation data into the Postgres buffer cache, reducing access times for frequently queried tables
  </Card>

  <Card title="pg_repack" icon="table" href="/extensions/pg_repack">
    Lets you remove bloat from tables and indexes, and optionally restore the physical order of clustered indexes
  </Card>

  <Card title="pg_roaringbitmap" icon="table" href="https://github.com/ChenHuajun/pg_roaringbitmap">
    Implements Roaring Bitmaps in Postgres for efficient storage and manipulation of bit sets
  </Card>

  <Card title="rum" icon="table" href="https://github.com/postgrespro/rum">
    Provides an access method to work with a RUM index, designed to speed up full-text searches
  </Card>


</CardGroup>

## Metrics

<CardGroup>


  <Card title="neon" icon="/images/icons/metrics.svg" href="/extensions/neon">
    Provides functions and views designed to gather Neon-specific metrics
  </Card>

  <Card title="pg_stat_statements" icon="/images/icons/metrics.svg" href="/extensions/pg_stat_statements">
    Tracks planning and execution statistics for all SQL statements executed, aiding in performance analysis and tuning
  </Card>

  <Card title="pgstattuple" icon="/images/icons/metrics.svg" href="/extensions/pgstattuple">
    Offers functions to show tuple-level statistics for tables, helping identify bloat and efficiency opportunities
  </Card>

  <Card title="tsm_system_rows" icon="/images/icons/metrics.svg" href="https://www.postgresql.org/docs/16/tsm-system-rows.html">
    Provides a table sampling method that selects a fixed number of table rows randomly
  </Card>

  <Card title="tsm_system_time" icon="/images/icons/metrics.svg" href="https://www.postgresql.org/docs/16/tsm-system-time.html">
    Offers a table sampling method based on system time, enabling consistent sample data retrieval over time
  </Card>


</CardGroup>

## Orchestration

<CardGroup>


  <Card title="tcn" icon="/images/icons/gear.svg" href="https://www.postgresql.org/docs/16/tcn.html">
    Provides a trigger function to notify listeners of changes to a table, allowing applications to respond to changes in the database
  </Card>

  <Card title="pg_partman" icon="/images/icons/gear.svg" href="https://github.com/pgpartman/pg_partman">
    A partition manager extension that enables creating and managing time-based and number-based table partition sets in Postgres
  </Card>


</CardGroup>

## Procedural languages

<CardGroup>


  <Card title="plcoffee" icon="/images/icons/binary-code.svg" href="https://coffeescript.org/">
    Enables writing functions in CoffeeScript, a Javascript dialect with a syntax similar to Ruby
  </Card>

  <Card title="plls" icon="/images/icons/binary-code.svg" href="https://livescript.net/">
    Enables writing functions in LiveScript, a Javascript dialect that serves as a more powerful successor to CoffeeScript
  </Card>

  <Card title="plv8" icon="/images/icons/binary-code.svg" href="https://github.com/plv8/plv8/">
    A Postgres procedural language powered by V8 Javascript Engine for writing functions in Javascript that are callable from SQL
  </Card>

  <Card title="plpgsql" icon="/images/icons/binary-code.svg" href="https://www.postgresql.org/docs/16/plpgsql.html">
    The default procedural language for Postgres, enabling the creation of complex functions and triggers
  </Card>


</CardGroup>

## Query optimization

<CardGroup>


  <Card title="hypopg" icon="/images/icons/find-replace.svg" href="https://hypopg.readthedocs.io/en/rel1_stable/">
    Provides the ability to create hypothetical (virtual) indexes in Postgres for performance testing
  </Card>

  <Card title="pg_hint_plan" icon="/images/icons/find-replace.svg" href="https://github.com/ossc-db/pg_hint_plan">
    Allows developers to influence query plans with hints in SQL comments, improving performance and control over query execution
  </Card>

  <Card title="online_advisor" icon="/images/icons/find-replace.svg" href="/extensions/online_advisor">
    Recommends indexes, extended statistics, and prepared statements based on your actual query workload
  </Card>


</CardGroup>

## Scientific computing

<CardGroup>


  <Card title="cube" icon="/images/icons/atom.svg" href="/extensions/cube">
    Implements the cube data type for representing multidimensional cubes in Postgres
  </Card>

  <Card title="rdkit" icon="/images/icons/atom.svg" href="https://github.com/rdkit/rdkit">
    Integrates the RDKit cheminformatics toolkit with Postgres, enabling chemical informatics operations directly in the database
  </Card>

  <Card title="seg" icon="/images/icons/atom.svg" href="https://www.postgresql.org/docs/16/seg.html">
    Implements the seg data type for storage and manipulation of line segments or floating-point intervals, useful for representing laboratory measurements
  </Card>

  <Card title="unit" icon="/images/icons/atom.svg" href="https://github.com/df7cb/postgresql-unit">
    Implements a data type for SI units, plus byte, for storage, manipulation, and calculation of scientific units
  </Card>


</CardGroup>

## Search

<CardGroup>


  <Card title="citext" icon="search" href="/extensions/citext">
    Provides a case-insensitive character string type that internally calls lower when comparing values in Postgres
  </Card>

  <Card title="dict_int" icon="search" href="/extensions/dict_int">
    Provides a text search dictionary template for indexing integer data in Postgres
  </Card>

  <Card title="fuzzystrmatch" icon="search" href="/extensions/fuzzystrmatch">
    Provides several functions to determine similarities and distance between strings in Postgres
  </Card>

  <Card title="pg_search" icon="search" href="/extensions/pg_search">
    An Elasticsearch alternative for full-text search and analytics on Postgres
  </Card>

  <Card title="pg_trgm" icon="search" href="/extensions/pg_trgm">
    Provides functions and operators for determining the similarity of alphanumeric text based on trigram matching, and index operator classes for fast string similarity search
  </Card>

  <Card title="prefix" icon="search" href="https://github.com/dimitri/prefix">
    A prefix range module that supports efficient queries on text columns with prefix-based searching and matching capabilities
  </Card>

  <Card title="unaccent" icon="search" href="/extensions/unaccent">
    A text search dictionary that removes accents from characters, simplifying text search in Postgres
  </Card>


</CardGroup>

## Security

<CardGroup>


  <Card title="anon" icon="/images/icons/data.svg" href="/extensions/postgresql-anonymizer">
    Provides data masking and anonymization capabilities for protecting personally identifiable information (PII) and sensitive data in Postgres databases
  </Card>

  <Card title="pg_session_jwt" icon="/images/icons/check.svg" href="/extensions/pg_session_jwt">
    Handle authenticated sessions through JWTs for use with RLS policies
  </Card>

  <Card title="pgcrypto" icon="/images/icons/check.svg" href="/extensions/pgcrypto">
    Offers cryptographic functions, allowing for encryption and hashing of data within Postgres
  </Card>

  <Card title="pgjwt" icon="/images/icons/check.svg" href="https://github.com/michelp/pgjwt">
    Implements JSON Web Tokens (JWT) in Postgres, allowing for secure token creation and verification
  </Card>


</CardGroup>

## Tooling / Admin

<CardGroup>


  <Card title="autoinc" icon="/images/icons/wrench.svg" href="https://www.postgresql.org/docs/current/contrib-spi.html">
    Provides an autoinc() function that stores the next value of a sequence into an integer field
  </Card>

  <Card title="hypopg" icon="/images/icons/wrench.svg" href="https://hypopg.readthedocs.io/en/rel1_stable/">
    Provides the ability to create hypothetical (virtual) indexes in Postgres for performance testing
  </Card>

  <Card title="insert_username" icon="/images/icons/wrench.svg" href="https://www.postgresql.org/docs/current/contrib-spi.html">
    Automatically inserts the username of the person executing an insert operation into a specified table in Postgres
  </Card>

  <Card title="lo" icon="/images/icons/wrench.svg" href="https://www.postgresql.org/docs/16/lo.html">
    Provides support for managing large objects (LOBs) in Postgres, including a data type lo and a trigger lo_manage
  </Card>

  <Card title="neon_utils" icon="/images/icons/wrench.svg" href="/extensions/neon-utils">
    Provides a function for monitoring how Neon's Autoscaling feature allocates compute resources in response to workload
  </Card>

  <Card title="pg_cron" icon="/images/icons/wrench.svg" href="/extensions/pg_cron">
    Lets you schedule and manage periodic jobs directly in your Postgres database
  </Card>

  <Card title="pgtap" icon="/images/icons/wrench.svg" href="https://pgtap.org/documentation.html">
    A unit testing framework for Postgres, enabling sophisticated testing of database queries and functions
  </Card>

  <Card title="refint" icon="/images/icons/wrench.svg" href="https://www.postgresql.org/docs/current/contrib-spi.html">
    Provides functions for maintaining foreign key constraints
  </Card>


</CardGroup>

## Remote data access

<CardGroup>


  <Card title="dblink" icon="/images/icons/queries.svg" href="/extensions/dblink">
    Provides functions for accessing and manipulating data in remote databases from within Postgres
  </Card>

  <Card title="postgres_fdw" icon="/images/icons/queries.svg" href="/extensions/postgres_fdw">
    Access data in remote Postgres databases as local tables
  </Card>


</CardGroup>
