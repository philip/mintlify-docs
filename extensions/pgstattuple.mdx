...
<Warning title="Resource Intensive Query">
...
- **Combine with `pg_repack`:** For online table and index reorganization to remove bloat without extensive locking (unlike `VACUUM FULL` or `REINDEX`), consider the [`pg_repack` extension](/extensions/pg_repack).
