...
- A source [Neon project](/manage/projects#create-a-project) with a database containing the data you want to replicate. If you're just testing this out and need some data to play with, you run the following statements from the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or an SQL client such as [psql](/connect/query-with-psql-editor) to create a table with sample data:
...
You can verify that logical replication is enabled by running the following query from the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or an SQL client such as [psql](/connect/query-with-psql-editor):
...
It's recommended that you create a dedicated Postgres role for replicating data. The role must have the `REPLICATION` privilege. The default Postgres role created with your Neon project and roles created using the Neon CLI, Console, or API are granted membership in the [neon_superuser](/manage/roles#the-neonsuperuser-role) role, which has the required `REPLICATION` privilege.
...
The following Neon API method creates a role. To view the API documentation for this method, refer to the [Neon API reference](/reference/cli-roles).
...
2. Enter the connection details for your Neon database. You can find your database connection details by clicking the **Connect** button on your **Project Dashboard**.
   ...
   ![Airbyte Create a source](/images/docs/guides/airbyte_create_source.png)
...
4. Select an SSL mode. You will most frequently choose `require` or `verify-ca`. Both of these options always require encryption. The `verify-ca` mode requires a certificate. Refer to [Connect securely](/connect/connect-securely) for information about the location of certificate files you can use with Neon.
5. Under **Advanced**:
...
![Airbyte Snowflake destination](/images/docs/guides/airbyte_snowflake_destination.png)
...
![Airbyte Snowflake verify replication](/images/docs/guides/airbyte_snowflake_verify.png)
