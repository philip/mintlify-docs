...
<Card title="Learn how platforms embed Neon">
[Retool manages 300k+ Postgres databases](https://neon.com/blog/how-retool-uses-retool-and-the-neon-api-to-manage-300k-postgres-databases) and [Koyeb offers serverless Postgres](https://www.koyeb.com/blog/serverless-postgres-public-preview) using Neon.
</Card>
...
- **Independent recovery**: Operations like instant [point-in-time restore](/guides/branch-restore) work at the branch level. In a project-per-user model, you can restore individual customer databases without impacting others.
...
1. Generate a Neon API key from your Neon account settings. See [Creating API keys](/manage/api-keys#creating-api-keys).
...
- [@neondatabase/toolkit](/reference/neondatabase-toolkit) (supports the Neon API for platform operations and the [Neon serverless driver](/serverless/serverless-driver) for connecting and running SQL queries)
...
For more on autoscaling, see [Autoscaling](/introduction/autoscaling) and [Scale to zero](/introduction/scale-to-zero).
...
For detailed information about configuring limits, see [Configure consumption limits](/guides/consumption-limits).
...
For details on querying metrics, see [Query consumption metrics](/guides/consumption-metrics).
...
Be aware of the [connection limits](/connect/connection-pooling#connection-limits-without-connection-pooling) associated with each compute size. Connection pooling allows for significantly more concurrent connections. If you expect a high number of connections, we recommend using a pooled connection string. To learn more, see [Connection pooling](/connect/connection-pooling).

### Reserved names

Neon reserves certain names for roles and databases. See [Reserved role names](/manage/roles#reserved-role-names) and [Reserved database names](/manage/databases#reserved-database-names).
...
See [Consumption polling](/guides/consumption-metrics#consumption-polling) for more details.

### Staying informed

Monitor these resources for updates that could impact your integration:

- [Neon Roadmap](/introduction/roadmap) for recent deliveries and upcoming features
- [Neon Changelog](/changelog) for product updates
- [Neon Status Page](https://neonstatus.com/) for platform status
- [RSS Feeds](/reference/feeds) for all of the above

## Advanced features

### Isolated development environments

Neon's [branching](/introduction/branching) feature lets your users create isolated copies of their database for development and testing. Branches are copy-on-write clones that initially share data with their parent, though storage costs accumulate as changes are made to the branch.
...
This is particularly valuable for platforms where users need to test schema changes or experiment with data safely. Branching is fully supported by the [Neon API](https://api-docs.neon.tech/reference/getting-started-with-neon-api). For examples, see [Branching with the Neon API](/manage/branches#branching-with-the-neon-api).

### Schema migrations

If you're managing the same schema across many user databases, consider using tools like:

- [Drizzle migrations](/guides/drizzle-migrations)
- [Prisma migrations](/guides/prisma-migrations)
- [Flyway](/guides/flyway)
- [Liquibase](/guides/liquibase)
...
<CardGroup>
  ...
  <Card title="Neon API Reference" icon="/images/icons/transactions.svg" href="/reference/api-reference">
    Explore all available API endpoints
  </Card>

  <Card title="Talk to Sales" icon="/images/icons/todo.svg" href="/contact-sales">
