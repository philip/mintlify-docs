...
For more information about Neon's read replica feature, see [Read replicas](/introduction/read-replicas).
...
- An application that uses Prisma with a Neon database. If you haven't set up Prisma yet, see [Connect from Prisma to Neon](/guides/prisma).
...
5. Specify the **Compute size settings** options. You can configure a **Fixed Size** compute with a specific amount of RAM (the default) or enable autoscaling by configuring a minimum and maximum compute size. You can also configure the **Scale to zero** setting, which controls whether your read replica compute is automatically suspended due to inactivity after 5 minutes.
   <Note>
   The compute size configuration determines the processing power of your database. More memory means more processing power but also higher compute costs. For information about compute costs, see [Billing metrics](/introduction/billing).
   </Note>
...
Alternatively, you can create read replicas using the [Neon API](https://api-docs.neon.tech/reference/createprojectendpoint) or [Neon CLI](/reference/cli-branches#create).
...
<CardGroup>
  ...
  <Card title="Prisma read replicas demo" icon="/images/technology-logos/github.svg" href="https://github.com/prisma/read-replicas-demo">
