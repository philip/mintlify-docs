...
- [What connection pooling is and why it mattered for serverless](#the-core-change-connection-pooling)
- [The difference between "Classic Serverless" and Vercel's "Fluid compute"](#the-two-scenarios-classic-serverless-versus-fluid-compute)
- [How TCP, HTTP, and WebSocket connections compare on latency](#comparison-of-connection-methods)
- [Our recommendation for connecting from Vercel](#our-recommendation)
</Card>
...
- [Benchmarking latency](/guides/benchmarking-latency)
</Card>
</CardGroup>
...
- **The solution (HTTP/WebSocket):** To solve this, Neon provides the [@neondatabase/serverless](/serverless/serverless-driver) driver, which connects over HTTP or WebSockets. These protocols have _fewer setup roundtrips_ (~3-4), making them much faster _for the first query_.
...
<Note title="A note on benchmarking">
...
Continue using the [@neondatabase/serverless](/serverless/serverless-driver) driver. Its HTTP-based connection is optimized for low-latency "first queries," which is the most important metric in that environment.
