...
<Note title="Beta">
The **Postgres logs export** is in Beta. Share your feedback on [Discord](https://discord.gg/92vNTzKDGp) or via the [Neon Console](https://console.neon.tech/app/projects?modal=feedback).
</Note>
...
- You have a Neon account and project. If not, see [Sign up for a Neon account](/get-started/signing-up).
...
For a fully managed solution, see the dedicated [Grafana Cloud integration guide](/guides/grafana-cloud).
...
2. Once signed in, you'll need your New Relic license key for authentication.
   ...
   ![New Relic profile menu showing API Keys option](/images/docs/guides/new_relic_api_keys.png) 3. Click **Create a key** → choose **Ingest - License**. Copy the key immediately (you can't view it again later).

   ![New Relic API Keys page showing license key types](/images/docs/guides/new_relic_copy_key.png)
...
2. Locate the **OpenTelemetry** card and click **Add**.
   ...
   ![open telemetry integration card](/images/docs/guides/open_telemetry_card.png)

   The sidebar form opens for you to enter your platform's details.

   ![OpenTelemetry integration configuration form](/images/docs/guides/opentelemetry_config_form.png)
...
2. **Enter your Endpoint URL** based on your platform:
   ...
   **For Grafana Cloud**:
   ...
   - See the [Grafana Cloud guide](/guides/grafana-cloud) for details
...
3. **Create dashboards**: You can visualize using [Grafana Drilldown apps](https://grafana.com/docs/grafana/latest/explore/simplified-exploration/) or use the dashboard JSON from [Grafana Cloud Documentation](/guides/grafana-cloud#create-a-monitoring-dashboard)
...
![Neon metrics appearing in New Relic](/images/docs/guides/new_relic_metrics_success.png)

_Postgres logs flowing into New Relic_
![Neon PostgreSQL logs appearing in New Relic](/images/docs/guides/new_relic_logs_success.png)

**Find your data under APM & Services**
![Multiple Neon services in New Relic APM & Services](/images/docs/guides/new_relic_services.png)
...
Additionally, if you are setting up the OpenTelemetry integration for a project with an inactive compute, you'll need to activate the compute before it can send data. To activate it, simply run a query from the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or any connected client.
</Note>
...
For a complete list of all metrics and log fields exported by Neon, see the [Metrics and logs reference](/reference/metrics-logs).
