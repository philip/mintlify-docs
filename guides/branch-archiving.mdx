...
<a href="/introduction/architecture-overview#archive-storage">Archive storage</a>
  <a href="/reference/cli-branches#list">Branches list command (Neon CLI)</a>
...
- Is a **protected branch** ([learn more](/guides/protected-branches)).
...
- [Connecting to or querying the branch from a client or application](/connect/connect-from-any-app)
- [Querying the branch from the Neon SQL Editor](/get-started/query-with-neon-sql-editor)
- [Viewing the branch on the Tables page in the Neon Console](/guides/tables)
- [Creating a child branch](/manage/branches#create-a-branch)
- [Creating a role on a branch](/manage/roles#create-a-role)
- [Creating a database on a branch](/manage/databases#create-a-database)
- [Reset the branch from its parent](/manage/branches#reset-a-branch-from-parent)
- [Performing a restore operation on a branch](/guides/branch-restore)
- [Setting the branch as protected](/guides/protected-branches)
- Running [Neon CLI](/reference/neon-cli) commands or [Neon API](https://api-docs.neon.tech/reference/getting-started-with-neon-api) calls that access the branch
...
![the archive icon shown on a branch in the branches list page](/images/docs/guides/archived_branch_icon.png)

If you select an archived branch on the **Branches** page to view its details, you can see when the branch was archived:

![the archive status shown on a branch in the branch detail page](/images/docs/guides/archived_branch_details.png)
...
If you know when a branch should be deleted, set an expiration date rather than wait for automatic archiving. This guarantees automatic deletion at the specified time and works well for CI/CD pipelines and temporary environments. See [Branch expiration](/guides/branch-expiration) for details.

## Disabling branch archiving

You cannot fully disable branch archiving, but you can prevent a branch from being archived by defining it as a **protected branch**. For instructions, see [Set a branch as protected](/manage/branches#set-a-branch-as-protected). Protected branches are supported on Neon paid plans.
...
For related information, see [System operations](/manage/operations).
...
<Tab title="CLI">
The Neon CLI [branches list](/reference/cli-branches#list) command shows a branch's `Current State`. Branch states include:
