...
<Info title="Private Networking availability">
Private Networking is available on Neon's [Scale](/introduction/plans#scale) plan. If you're on a different plan, you can request a trial from the **Network Security** page in your project's settings.
</Info>
...
![Neon Private Networking diagram](/images/docs/guides/neon_private_access.jpg)

## Prerequisites

- You must be a Neon [Business](/introduction/plans#business) and [Scale](/introduction/plans#scale) account user, and your user account must be [Neon organization](/manage/organizations) Admin account. You'll encounter an access error if you attempt the setup from a personal Neon account or on a Neon plan that does not offer Private Networking.
- **Ensure that your client application is deployed on AWS in the same region as the Neon database you plan to connect to.** The Private Networking feature is available in all [Neon-supported AWS regions](/introduction/regions#aws-regions). Both your private access client application and Neon database must be in one of these regions.
...
- Install the Neon CLI. You will use it to add your VPC endpoint ID to your Neon organization. For installation instructions, see [Neon CLI — Install and connect](/reference/cli-install).
...
## Create an AWS VPC endpoint
    ...
    <Warning>
    Do not enable **private DNS names** for the VPC endpoint until [Step 3](/guides/neon-private-networking#enable-private-dns). You must add the VPC endpoint to your Neon organization first, as described in [Step 2](/guides/neon-private-networking#add-your-vpc-endpoint-id-to-your-neon-organization).
    </Warning>
    ...
    1. Go to the AWS **VPC > Endpoints** dashboard and select **Create endpoint**. Make sure you create the endpoint in the same VPC as your client application.
       ...
       ![VPC Dashboard](/images/docs/guides/pl_vpc_dashboard.png)
    ...
    1. For **Type**, select the **Endpoint services that use NLBs and GWLBs** category.
       ...
       ![VPC Create endpoint](/images/docs/guides/pl_vpc_create_endpoint.png)
    ...
    1. Click **Verify service**. If successful, you should see a `Service name verified` message.
        ...
        ![VPC Create endpoint](/images/docs/guides/pl_vpc_service_name_verified.png)
    ...
    1. Click **Create endpoint** to complete the setup of the endpoint service.
        ...
        ![VPC Create endpoint](/images/docs/guides/pl_vpc_network_settings.png)

    1. Note your **VPC Endpoint ID**. You will need it in the next step.

        ![VPC Create endpoint](/images/docs/guides/pl_vpc_endpoint_id.png)
...
## Add your VPC Endpoint ID to your Neon organization
    ...
    In the following example, the VPC endpoint ID is assigned to a Neon organization in the specified AWS region using the [neon vpc endpoint](/reference/cli-vpc#the-vpc-endpoint-subcommand) command.
    ...
    You can use the [Assign or update a VPC endpoint](https://api-docs.neon.tech/reference/assignorganizationvpcendpoint) API to assign a VPC endpoint ID to a Neon organization. You will need to provide your Neon organization ID, region ID, VPC endpoint ID, and a [Neon API key](/manage/api-keys).
...
## Enable Private DNS
    ...
    1. Save your changes.
       ![Enable private DNS](/images/docs/guides/pl_enable_private_dns.png)
...
## Restrict public internet access
    ...
    To block access via the Neon CLI, use the [neon projects update](/reference/cli-projects#update) command with the `--block-public-connections` option.
...
After adding a restriction, you can check the status of the VPC endpoint to view the restricted project using the [vpc endpoint status command](/reference/cli-vpc#the-vpc-endpoint-subcommand). You will need to provide your VPC endpoint ID, region ID, and Neon organization ID.
...
The Neon API supports managing project restrictions using the [Assign or update a VPC endpoint restriction](https://api-docs.neon.tech/reference/assignprojectvpcendpoint) endpoint. You will need to provide your VPC endpoint ID, Neon project ID, and a [Neon API key](/manage/api-keys).
...
For more details and examples, see [Neon CLI commands — vpc](/reference/cli-vpc).
