...
In the detailed view for any child branch, you can check the schema differences between the selected branch and its parent. Use this view to verify the state of these schemas before you [Reset from parent](/guides/reset-from-parent).

### Compare to another branch's history

Built into the Time Travel assist editor, you can use Schema Diff to help when restoring branches, letting you compare states of your branch against its own or another branch's history before you complete a [branch restore](/guides/branch-restore) operation.
...
![Schema diff from branches page](/images/docs/guides/schema_diff_compare_parent.png)

### From the Restore page

Just like with [Time Travel Assist](/guides/branch-restore#using-time-travel-assist), your first step is to choose the branch you want to restore, then choose where you want to restore from: **From history** (its own history) or **From another branch** (from another branch's history).
...
![schema diff results](/images/docs/guides/schema_diff_result.png)
...
To find out what other comparisons you can make, see [Neon CLI commands — branches](/reference/cli-branches#schema-diff) for full documentation of the command.
...
| `Authorization`    | Bearer token for API access (your [Neon API key](/manage/api-keys))      | Yes      | `$NEON_API_KEY`            |
...
- `timestamp` or `lsn` / `base_timestamp` or `base_lsn` values can be used to compare schemas as they existed as a precise time or [LSN](/reference/glossary#lsn).  
...
Neon supports a [Schema Diff GitHub Action](/guides/branching-github-actions#schema-diff-action) that performs a database schema diff on specified Neon branches for each pull request and writes a comment to the pull request highlighting the schema differences.
...
To learn more, see the [Schema Diff GitHub Action](/guides/branching-github-actions#schema-diff-action).

## Tutorial

For a step-by-step guide showing you how to compare two development branches using Schema Diff, see [Schema diff tutorial](/guides/schema-diff-tutorial).
