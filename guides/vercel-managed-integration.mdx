...
- [What the Vercel-Managed Integration is](#about-this-integration)
- [How to install it from the Vercel Marketplace](#installation-walkthrough)
- [How (and why) to enable automated Preview Branching](#enable-automated-preview-branching-recommended)
- [Where to manage billing and configuration](#managing--billing)
</Card>
<Card title="Related topics">
- [Neon-Managed Integration](/guides/neon-managed-vercel-integration)
...
<Note title="Who should use this path?">
...
![Create a New Neon Account](/images/docs/guides/vercel_install_neon_modal_new_account.png)
...
1. In **Storage → `<your database>` → Connect Project** choose the Vercel project and the environments that should receive database variables (Development, Preview, Production).
   ...
   ![Connect a Vercel Project](/images/docs/guides/vercel_native_connect_project.png)
...
2. (Optional) Under **Advanced Options → Deployments Configuration** enable **Preview** to turn on _Preview Branching_ (see next section).
   ...
   ![Vercel deployment configuration](/images/docs/guides/vercel_native_deployments_configuration.png)

3. Click **Connect**.

<Tip title="Environment variable prefix">You can add a prefix if you have multiple databases in the same project, for example `PRIMARY_`.</Tip>
...
- Cleanup happens when deployments are deleted, which you can configure using [Vercel's retention policy settings](https://vercel.com/docs/deployment-retention). By default, Pre-Production Deployments (preview environments) are retained for 180 days:
  ...
  ![Vercel retention policy defaults](/images/docs/guides/vercel_retention_policy_defaults.png)

<Note>
This deployment-based cleanup differs from the [Neon-Managed Integration](/guides/neon-managed-vercel-integration), which deletes branches when Git branches are deleted.
</Note>
...
- If the destination's plan doesn't support the project's requirements (autoscaling limits, point-in-time [restore window](/introduction/restore-window), etc.), you'll be prompted to upgrade.
...
- [Neon Console](/manage/branches#delete-a-branch) - Individual or bulk deletion
- [Neon CLI](/reference/cli-branches#delete) - Command line management
- [Neon API](/manage/branches#delete-a-branch-with-the-api) - Programmatic cleanup

<Note title="Unused branches are archived">
Branches you don't delete are eventually archived, consuming archive storage space. See [Branch archiving](/guides/branch-archiving).
</Note>
