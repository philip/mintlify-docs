...
description: >-
  Covers the setup of connecting a Rust application to a Neon Postgres database
  using the rust-postgres and tokio-postgres crates, including project creation
...
import RustPrompt from '/snippets/prompts/rust-prompt.mdx';
...
- Start a transaction to insert multiple book records in a single operation.
  ...
  <Info title="Why use a transaction for inserting multiple rows?">
  Unlike database drivers in some other languages that offer a single high-level method for bulk inserts (like [Python's](/guides/python#create-a-table-and-insert-data) `executemany` in `psycopg2`), the idiomatic Rust approach is to loop through the data inside a transaction.
...
<CardGroup>
  ...
  <Card title="Get started with Rust and Neon using postgres" icon="/images/technology-logos/github.svg" href="https://github.com/neondatabase/examples/tree/main/with-rust-postgres">
    Get started with Rust and Neon using the synchronous postgres crate
  </Card>

  <Card title="Get started with Rust and Neon using tokio-postgres" icon="/images/technology-logos/github.svg" href="https://github.com/neondatabase/examples/tree/main/with-rust-tokio-postgres">
