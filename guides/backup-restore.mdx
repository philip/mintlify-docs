...
<Note title="Snapshots in Beta">
The **Snapshots** feature is now in Beta and available to all users. Snapshot limits: 1 on the Free plan and 10 on paid plans. Automated backup schedules are available on paid plans except for the Agent plan. If you need higher limits, please reach out to [Neon support](/introduction/support).
...
![Backup and restore UI](/images/docs/guides/backup_restore_ui.png)
...
> Instant restore is only supported for root branches. Typically, this is your project's `production` branch. [Learn more](/manage/branches#root-branch).

<Tabs>

<Tab title="Console">

You can restore from any time that falls within your project's [restore window](/introduction/restore-window).

1. **Select a time**

   Click the date & time selector, choose a date & time, and click **Restore**.

   ![Backup and restore select a restore time](/images/docs/guides/backup_restore_select_time.png)
   ...
   ![Backup and restore preview data](/images/docs/guides/backup_restore_preview_modal.png)
...
2. **Preview the data**
   ...
   ![Backup and restore preview data options](/images/docs/guides/backup_restore_preview_options.png)
...
3. **Restore**
   ...
   ![Backup branch on the Branches page](/images/docs/guides/backup_restore_backup_branch.png)

   For information about removing backup branches, see [Deleting backup branches](/introduction/branch-restore#deleting-backup-branches).
...
For full CLI documentation for `branches restore`, see [branches restore](/reference/cli-branches#restore).
...
![Backup branch on the Branches page](/images/docs/guides/backup_restore_create_snapshot.png)

</Tab>

<Tab title="API">

You can create a snapshot from a branch using the [Create snapshot](https://api-docs.neon.tech/reference/createsnapshot) endpoint. A snapshot can be created from a specific timestamp (RFC 3339 format) or LSN (for example 16/B3733C50) within the branch's [restore window](/introduction/restore-window). The `timestamp` and `lsn` parameters are mutually exclusive; you can use one or the other, not both.
...
1. **Open the schedule editor**
   ...
   ![Edit schedule button on Backup & restore page](/images/docs/guides/edit_snapshot_schedule.png)
...
2. **Select a schedule frequency**
   ...
   ![Schedule frequency options dropdown](/images/docs/guides/snapshot_schedule_menu.png)
...
![Edit backup schedule modal](/images/docs/guides/edit_backup_schedule_modal.png)
...
1. Locate the snapshot you want to use and click **Restore → One-step restore**.
   ...
   ![One step restore option](/images/docs/guides/one_step_restore.png)
...
2. The **One-step restore** modal explains the operation:
   ...
   ![One step restore confirmation modal](/images/docs/guides/one_step_restore_modal.png)
...
3. Your branch is immediately restored to the snapshot state, and the `<branch_name>_old` branch is created, which you'll find on the **Branches** page in the Neon Console, as shown here:
   ![Branches page that shows the backup branch](/images/docs/guides/one_step_restore_branches_page.png)
...
1. Locate the snapshot you want to use and click **Restore → Multi-step restore**.
   ![Multi-step restore option](/images/docs/guides/multi_step_restore.png)
2. The **Multi-step restore** modal explains the operation:
   ...
   ![Multi-step restore confirmation modal](/images/docs/guides/multi_step_restore_modal.png)
...
3. Clicking **Restore** creates the new branch with the restored data and directs you to the **Branch overview** page where you can:
   ...
   ![Branch overview page](/images/docs/guides/branch_overview_page.png)
...
- **Reset from parent is unavailable on child branches for up to 24 hours after restoring a parent from a snapshot.** When you restore a branch from a snapshot, any child branches of that restored branch cannot use the [Reset from parent](/guides/reset-from-parent) feature for up to 24 hours.
