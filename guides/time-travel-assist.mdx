...
To learn more about the restore window, including how to configure it and plan limits, see [Restore window](/introduction/restore-window).
...
![time travel error message](/images/docs/guides/time_travel_error.png)
...
![time travel from sql editor](/images/docs/guides/time_travel_sql.png)

### Time Travel Assist with instant restore

Time Travel Assist is also available from the **Restore** page, as part of the [Instant restore](/guides/branch-restore) feature. Before completing a restore operation, it's a good idea to use Time Travel Assist to verify that you've targetted the correct restore point.
...
![Time travel assist](/images/docs/guides/time_travel_assist.png)
...
1. Use the **Time Travel** (🕣) icon to enable querying against an earlier point in time.
   ...
   ![Time Travel toggle](/images/docs/get-started/time_travel_sql_editor.png)
...
1. Select the branch you want to query against, then select a timestamp, the same as you would to [Restore a branch](#restore-a-branch-to-an-earlier-state).
   ...
   ![time travel selection](/images/docs/guides/time_travel_restore_select.png)
   ...
   ![Time travel assist](/images/docs/guides/time_travel_show_selected.png)
...
1. Check that you have the right database selected to run your query against. Use the database selector under the SQL Editor to switch to a different database for querying against.
   ...
   ![time travel select db](/images/docs/guides/time_travel_db_select.png)
...
1. Write your read-only query in the editor, then click **Query at timestamp** to run the query. You don't have to include time parameters in the query; the query is automatically targeted to your selected timestamp.
   ...
   ![time travel results](/images/docs/guides/time_travel_results.png)
...
Read more about getting connection strings from the CLI in [Neon CLI commands — connection-string](/reference/cli-connection-string), and more about setting contexts in [CLI - set-context](/reference/cli-set-context).
...
- Ephemeral endpoints are created with a .50 CU compute size (2 GB of RAM). This is Neon's second smallest compute size. For more about compute sizes in Neon, see [How to size your compute](/manage/computes#how-to-size-your-compute).
