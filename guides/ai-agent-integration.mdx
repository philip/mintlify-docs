...
<Card title="Learn from other agent platform builders" isIntro>
[Anything manages per-agent isolation at scale](https://neon.com/blog/anything-neon-integration), [Databutton built full-stack AI agents with Postgres and Auth](https://neon.com/blog/databutton-neon-integration), and [Dyad implemented database versioning for AI-generated apps](https://neon.com/blog/building-versioning-for-ai-generated-apps) using Neon.
</Card>

<Note>
**Prerequisites:** You must be enrolled in the [Neon Agent Plan](/introduction/agent-plan). If you haven't applied yet, visit [Neon for AI Agent Platforms](https://neon.com/use-cases/ai-agents).
</Note>
...
After enrolling in the [Neon Agent Plan](/introduction/agent-plan), you should have:
...
Keep your API keys secure. You'll use them for all API operations in this guide. If you do not have the API keys, see [Manage API keys](/manage/api-keys) for how to retrieve them.

### Project-per-tenant architecture

This integration uses a **project-per-tenant model**, where each tenant (user, app, or agent) gets its own dedicated Neon [project](/manage/overview) (containing branches, databases, roles, and computes). This provides complete data and resource isolation, makes consumption limits and billing straightforward, and aligns with how the Neon API is designed. For more on this database-per-tenant approach, see [Data Isolation at Scale](https://neon.com/use-cases/database-per-tenant).

<Tip>
For details about **Agent plan** structure, pricing, and benefits, refer to the [Neon Agent Plan](/introduction/agent-plan) docs.
</Tip>
...
For detailed quota examples and consumption limits, see [Configure consumption limits](/guides/consumption-limits).
...
For detailed information about configuring quotas and what happens when limits are reached, see [Configure consumption limits](/guides/consumption-limits).
...
<Tip>
**Transfer limits:** You can transfer up to 400 projects per API request. To create a personal API key, see [Create a personal API key](/manage/api-keys#create-a-personal-api-key).
</Tip>

For more details, see [Transfer projects between organizations](/manage/orgs-project-transfer).
...
Use PITR for recent history. The [restore window](/introduction/restore-window) differs between your two organizations:
...
Use snapshots (branches) for versions you want to keep beyond the [restore window](/introduction/restore-window):
...
For more details on using snapshots, see [Database versioning for AI agents](/ai/ai-database-versioning).
...
For complete details on parameters, pagination, response formats, and metric definitions, see [Query consumption metrics](/guides/consumption-metrics).
...
See [Configure consumption limits](/guides/consumption-limits) for details.
...
- **Monitor quotas**: Alert users at 80% and 95% of consumption limits. See [Query consumption metrics](/guides/consumption-metrics).
...
- **Connection pooling**: Provide [pooled database connection strings](/connect/connection-pooling) by default for bursty workloads.
- **Reserved names**: Avoid [reserved role names](/manage/roles#reserved-role-names) and [database names](/manage/databases#reserved-database-names).
...
- **[Neon Toolkit](/reference/neondatabase-toolkit)** (TypeScript): API client for management + serverless driver for queries; optimized for edge/serverless runtimes.
- **Other SDKs**: [Python SDK](/reference/python-sdk), [Go SDK](https://github.com/kislerdm/neon-sdk-go), [Node.js/Deno SDK](https://github.com/paambaati/neon-js-sdk). See [Neon SDKs](/reference/sdk) for all options.

## Cost management

- **Free organization**: No charges to you for up to 30,000 free tier projects (Neon-sponsored).
- **Paid organization**: Usage-based billing at $0.106 per compute unit hour, covered by your initial credits. See [Agent plan pricing](/introduction/agent-plan#pricing).
- **Monitor usage**: Track `active_time_seconds`, `compute_time_seconds`, `written_data_bytes`, `synthetic_storage_size_bytes` using [project metrics API](https://api-docs.neon.tech/reference/getconsumptionhistoryperproject). Poll every 15 minutes; doesn't wake computes. See [Query consumption metrics](/guides/consumption-metrics).
- **Set quotas**: Configure usage limits during [project creation](#provisioning-projects) or update later. See [Configure consumption limits](/guides/consumption-limits).
