...
- A Neon account. See [Sign up](/get-started/signing-up).
- A Neon project. See [Create your first project](/get-started/signing-up).
...
1. Select **Databases** from the sidebar and create a database named `blog`. For instructions, see [Create a database](/manage/databases#create-a-database).
1. Using the [Neon SQL Editor](/get-started/query-with-neon-sql-editor), add the following tables:
...
Learn how to use Liquibase with Neon's database branching feature to set up a developer workflow. See [Set up a developer workflow with Liquibase and Neon](/guides/liquibase-workflow).
