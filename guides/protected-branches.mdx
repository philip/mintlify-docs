...
- With additional configuration steps, you can apply IP Allow restrictions to protected branches only. The [IP Allow](/introduction/ip-allow) feature is available on the Neon [Scale](/introduction/plans) plan. See [below](#how-to-apply-ip-restrictions-to-protected-branches).
- Protected branches are not [archived](/guides/branch-archiving) due to inactivity.

The protected branches feature is available on Neon [paid plans](/introduction/plans).
...
2. Select **Branches** to view the branches for the project.
   ...
   ![Branch page](/images/docs/guides/ip_allow_branch_page.png)
...
4. On the branch page, click **Protect**.
   ...
   ![Set as protected](/images/docs/guides/ip_allow_set_as_protected.png)

5. In the **Set as protected** confirmation dialog, click **Set as protected** to confirm your selection.

   ![Set as protected confirmation](/images/docs/guides/ip_allow_set_as_protected_confirmation.png)

   Your branch is now designated as protected, as indicated by the protected branch shield icon, shown below.

   ![Branch page badge](/images/docs/guides/ip_allow_branch_badge.png)

   The protected branch designation also appears on your **Branches** page.

   ![Branches page badge](/images/docs/guides/ip_allow_branch_badge_2.png)
...
- The "new password" feature for child branches was released on July, 31, 2024. If you have existing CI scripts that create branches from protected branches, please be aware that passwords for matching Postgres roles on those newly created branches will now differ. If you depend on those passwords being the same, you'll need to make adjustments to get the correct connection details for those branches.
    - After a branch is created, the up-to-date connection string is returned in the output of the [Create Branch GitHub Action](/guides/branching-github-actions#create-branch-action).
    - The [Reset Branch GitHub Action](/guides/branching-github-actions#reset-from-parent-action) also outputs connection string values, in case you are using this action in your workflows.
    - The Neon CLI supports a [connection-string](/reference/cli-connection-string) command for retrieving a branch's connection string.
...
On plans that support it, you can use the protected branches feature in combination with Neon's [IP Allow](/introduction/ip-allow) feature to apply IP access restrictions to protected branches only. The basic setup steps are:
...
For details about specifying IP addresses, see [How to specify IP addresses](/manage/projects#how-to-specify-ip-addresses).
...
![IP Allow configuration](/images/docs/guides/ip_allow_protected_branches.png)
