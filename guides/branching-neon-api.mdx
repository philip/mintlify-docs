...
- The `endpoints` attribute creates a compute, which is required to connect to the branch. Neon supports `read_write` and `read_only` compute types. A branch can be created with or without a compute. You can specify `read_only` to create a [read replica](/guides/read-replica-guide).
...
For details on how to use this endpoint to restore a branch to its own or another branch's history, restore a branch to the head of its parent, and other restore options, see [Instant restore using the API](/introduction/branch-restore#how-to-use-instant-restore).
...
For more detailed information about branch expiration, including updating and removing expiration timestamps, see [Branch expiration](/guides/branch-expiration).
