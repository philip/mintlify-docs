...
1. Create the `users` table in your Neon database. You can do this via the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or by connecting to your Neon database from an SQL client such as [psql](/connect/query-with-psql-editor).
...
5. On the **Add Postgres CDC Source connector** page:
   ...
   - If you use Neon's **IP Allow** feature to limit IP addresses that can connect to Neon, you will need to add the Confluent cluster static IP addresses to your allowlist. For information about configuring allowed IPs in Neon, see [Configure IP Allow](/manage/projects#configure-ip-allow). If you do not use Neon's **IP Allow** feature, you can skip this step.
