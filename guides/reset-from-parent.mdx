...
- You can only reset a branch to the latest data from its parent. Point-in-time resets based on timestamp or LSN are possible using [Instant restore](/introduction/branch-restore), a similar feature, with some differences: instant restore leaves a backup branch and is in general is intended more for data recovery than development workflow.
...
For more details about branch expiration, see [branch expiration](/guides/branch-expiration).
...
![Reset from parent](/images/docs/manage/reset_from_parent.png)
...
Read more about performing branching actions from the CLI in [CLI - branches](/reference/cli-branches), and more about setting contexts in [CLI - set-context](/reference/cli-set-context).
...
For details, see [Instant restore using the API](/introduction/branch-restore#how-to-use-instant-restore)
