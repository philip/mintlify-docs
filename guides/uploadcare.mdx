...
4.  Note your **Public Key** and **Secret Key**. They are needed to interact with the Uploadcare API and widgets.
    ![Uploadcare API Keys](/images/docs/guides/uploadcare-api-keys.png)
...
1. You can run the create table statement using the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or from a client such as [psql](/connect/query-with-psql-editor) that is connected to your Neon database. Here is an example SQL statement to create a simple table for file metadata which includes a file ID, URL, user ID, and upload timestamp:
