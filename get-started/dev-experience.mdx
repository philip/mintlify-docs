...
You can build your database branching workflows using the [Neon CLI](/reference/neon-cli), [Neon API](https://api-docs.neon.tech/reference/getting-started-with-neon-api), or [GitHub Actions](/guides/branching-github-actions). For example, this example shows how to create a development branch from `main` with a simple CLI command:
...
For local development and CI pipelines, the [Neon CLI](/reference/neon-cli) provides a simple scripting interface that builds directly on the same API. Neon also provides native integrations for common workflows:
