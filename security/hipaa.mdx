...
Neon offers HIPAA compliance as a self-serve feature available to customers on the [Scale](/introduction/plans) plan.
...
For Neon project creation steps, see [Create a project](/manage/projects#create-a-project).
...
![Enable HIPAA option during project creation](/images/docs/security/enable_hipaa.png)
...
To create a new HIPAA-compliant Neon project via the [Neon CLI](/reference/neon-cli), use the `--hipaa` option with the `neon projects create` command, as shown below.
...
The `pgaudit` extension is preloaded on HIPAA-enabled Neon projects. For extension version information, see [Supported Postgres extensions](/extensions/pg-extensions).
...
- [Neon Auth](/neon-auth/overview) – Uses an authentication provider that is not covered under Neon’s HIPAA compliance.
- [Data API](/data-api/get-started) – Hosted outside Neon’s HIPAA-compliant infrastructure.
...
_This guide provides a high-level overview of Neon's HIPAA compliance efforts. For more details, please refer to your Business Associate Agreement (BAA) or contact us directly via our [support channels](/introduction/support)._
