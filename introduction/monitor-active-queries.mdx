...
![Neon active queries tab](/images/docs/introduction/active_queries.png)

The **Active queries** view is powered by the `pg_stat_activity` Postgres system view, which is available in Neon by default. To run custom queries against the data collected by `pg_stat_activity`, you can use the [Neon SQL Editor](/get-started/query-with-neon-sql-editor) or any SQL client, such as [psql](/connect/query-with-psql-editor).
...
<Note title="active queries retention">
