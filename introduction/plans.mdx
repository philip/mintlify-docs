...
<Note>
If you signed up with Neon through **Azure Marketplace**, see [Azure Marketplace](/introduction/billing-azure-marketplace) for plan and billing information specific to Azure users.
</Note>
...
<Info title="Building an agent platform?">
For AI agent platforms that provision thousands of databases, Neon offers an **Agent Plan** with custom resource limits and credits for **your** free tier. [Learn more](/introduction/agent-plan)
</Info>
...
| [Compliance and security](#compliance-and-security)   | —                            | Protected branches                   | SOC 2, ISO, GDPR, [HIPAA](/security/hipaa), Protected branches, IP Allow, Private Networking |
...
<Tip title="Optimize your costs">
Learn how to manage your Neon costs effectively with our [cost optimization guide](/introduction/cost-optimization), which covers strategies for compute, storage, branches, and data transfer.
</Tip>
...
A project is a container for your database environment. It includes your database, branches, compute resources, and more. Similar to a Git repository that contains code, artifacts, and branches, a project contains all your database resources. Learn more about [Neon's object hierarchy](/manage/overview).
...
- **Free**: 100 projects
- **Launch**: 100 projects
- **Scale**: 1,000 projects (soft limit; request more if needed via [support](/introduction/support))

### Branches

Each Neon project is created with a [root branch](/reference/glossary#root-branch), like the `main` branch in Git.  
Postgres objects (databases, schemas, tables, records, indexes, roles) are created on a branch.

You can create [child branches](/reference/glossary#child-branch) for testing, previews, or development.
...
> Projects can have multiple root branches, with limits based on your plan. See [Branch types: Root branch](/manage/branches#root-branch) for details.
...
> All computes in your project count toward usage. Each branch has a read-write compute by default; [read replicas](/reference/glossary#read-replica) add read-only computes.
...
- **[Root branches](/reference/glossary#root-branch)**: billed on actual data size (_logical data size_)
- **[Child branches](/reference/glossary#child-branch)**: billed on the minimum of the data changes since creation or the logical data size
...
<Warning title="Manage child branches to control storage costs">
...
- Set a [time to live](/guides/branch-expiration) on development and preview branches
- Delete child branches when they're no longer needed
- For production workloads, use a [root branch](/manage/branches#root-branch) instead; root branches are billed on your actual data size with no delta tracking overhead.
...
> Public network transfer includes data sent via [logical replication](/reference/glossary#logical-replication) to any destination, including other Neon databases.
...
See [Monitoring dashboard](/introduction/monitoring-page) for details.

### Metrics/logs export

Export metrics and Postgres logs to [Datadog](/guides/datadog) or any [OTel-compatible platform](/guides/opentelemetry).  
...
You can change your [restore window](#restore-window) to control how much change history you retain. See [Instant restore](/introduction/branch-restore) for details.

> The change history is a log of write operations in the form of Postgres [Write-Ahead Logs](/reference/glossary#write-ahead-logging-wal).
...
The restore window is configurable. Shortening it can reduce [instant restore](#instant-restore) storage costs but limits how far back you can restore. See [Restore window](/introduction/restore-window) for configuration details and more information.
...
Automated backup schedules are available on paid plans except for the Agent plan. See [Backup & restore](/guides/backup-restore) for details.
...
See [Neon Auth](/auth/overview) for more information.
...
- GDPR
- CCPA
- HIPAA ([additional charge](/security/hipaa))

Security features:

- [Protected branches](/guides/protected-branches): safeguards for critical data (available on **Launch** and **Scale**)
- [IP Allow](/introduction/ip-allow): restricts access to trusted IPs (available on **Scale**)
- [Private Networking](/guides/neon-private-networking): secure private connections via AWS PrivateLink (available on **Scale**)
...
See [Support](/introduction/support) for details.
...
![Usage metrics on the Projects page in the Neon Console](/images/docs/introduction/usage_metrics_console.png)
...
| **Network transfer** | Public network transfer <br/> Private network transfer (if [Private Networking](/guides/neon-private-networking) is enabled) |
...
| **Metric**                             | **Description**                                                                                                                                                                  |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Compute, CU-hour**                   | Total compute usage in **CU-hours** (Compute Unit hours). [Learn more](/introduction/plans#compute).                                                                        |
| **Extra branches, branch-month**       | Number of extra branches beyond your plan allowance, metered hourly. [Learn more](/introduction/plans#extra-branches).                                                      |
| **Instant restore storage, GB-month**  | Storage used for **instant restore**, billed per GB-month. [Learn more](/introduction/plans#instant-restore).                                                               |
| **Storage (root branches), GB-month**  | Data storage for root branches, billed per GB-month. [Learn more](/introduction/plans#storage).                                                                             |
| **Storage (child branches), GB-month** | Data storage for child branches (minimum of delta or logical size), billed per GB-month. [Learn more](/introduction/plans#storage).                                         |
| **Public network transfer**            | Outbound data transfer (egress) from your databases to the public internet. [Learn more](/introduction/plans#public-network-transfer).                                      |
| **Private network transfer**           | Bi-directional data transfer to and from your databases over private networking (for example, AWS PrivateLink). [Learn more](/introduction/plans#private-network-transfer). |
...
: Free: No, it's always enabled (5 min idle timeout).  
 ...
 Scale: Yes, fully configurable (1 minute to always-on). Learn more: [Scale to zero](/introduction/scale-to-zero)

What is autoscaling and how does it work?
: Autoscaling adjusts compute size based on load, between your set min/max limits. All plans support it, but maximum CU differs: Free up to 2 CU, Launch and Scale Scale up to 16 CU. Scale supports up to 56 CU for fixed-size computes. Learn more: [Autoscaling](/introduction/autoscaling)
...
: • Set a maximum autoscaling limit to cap compute size.  
 ...
 For more detailed strategies, see our [Cost optimization](/introduction/cost-optimization) guide.
...
: No. Archived branches are billed at the same rate as active branches. Neon automatically archives inactive branches to optimize storage resources and maintain a cost-efficient storage infrastructure. See [Branch archiving](/guides/branch-archiving) for details on how archiving works.
