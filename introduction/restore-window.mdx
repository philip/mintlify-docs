...
See [Cost optimization](/introduction/cost-optimization#instant-restore-storage).
...
See [Getting ready for production](/get-started/production-checklist#increase-your-projects-restore-window-to-7-days).

## Related features

The restore window determines what's available for these features:

- [Branching](/introduction/branching): Create branches from past states
- [Time Travel](/guides/time-travel-assist): Query historical data
- [Instant restore](/introduction/branch-restore): Restore a branch to any point within the window
- [Snapshots](/guides/backup-restore): Capture and restore from specific points

<Note>
The restore window is for branch point-in-time recovery (PITR), which restores data to a previous state. This is different from the deletion recovery period, which allows you to recover (undelete) a deleted project. For information about recovering deleted projects, see [Project recovery](/manage/projects#recover-a-deleted-project).
</Note>
