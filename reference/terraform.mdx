...
- **Provider upgrades**: When using `terraform init -upgrade` to update a custom Terraform provider, be aware that changes in the provider’s schema or defaults can lead to unintended resource replacements. This may occur when certain attributes are altered or reset. For example, fields previously set to specific values might be reset to `null`, forcing the replacement of the entire resource.
  ...
  - On Neon paid plans, you can enable branch protection to prevent unintended deletion of branches and projects. To learn more, see [Protected branches](/guides/protected-branches).
...
3.  **Neon API key:** Generate an API key from the Neon Console. Navigate to your Account Settings > API Keys. This key is required for the provider to authenticate with the Neon API. Learn more about creating API keys in [Manage API keys](/manage/api-keys).
...
![finding your organization ID from the settings page](/images/docs/manage/orgs_id.png)
...
- `region_id`: (Optional) The region where the project will be created (for example, `aws-us-east-1`).
  > For up-to-date information on available regions, see [Neon Regions](/introduction/regions).
...
- `name`: (Optional) Name for the new branch. If not specified, defaults to the branch ID. If provided, must be unique within the project and can be up to 256 characters. Cannot be empty or only whitespace. See [Branch naming requirements](/manage/branches#branch-naming-requirements).
...
2.  **Initialize terraform:** If you haven't already, or if you've just added the provider configuration, run:
    ...
    <Warning title="Important: Provider Upgrades">
...
1.  **Import the project:**
    ...
    You can retrieve the project ID via Neon Console/CLI/API. Learn more: [Manage projects](/manage/projects#project-settings)
...
2.  **Import the development branch:**
    ...
    You can retrieve the branch ID via Neon Console/CLI/API. Learn more: [Manage branches](/manage/branches)

    The following image shows the branch ID in the Neon Console:
    ![Neon Console Branch ID](/images/docs/guides/neon-console-branch-id.png)
...
3.  **Import the development compute endpoint:**
    ...
    You can retrieve the endpoint ID via Neon Console/CLI/API. Learn more: [Manage computes](/manage/computes).
    The following image shows the endpoint ID in the Neon Console:
    ![Neon Console Compute Endpoint ID](/images/docs/guides/neon-console-compute-endpoint-id.png)
...
<CardGroup>
  ...
  <Card title="Neon Postgres with Terraform and Go" icon="/images/technology-logos/github.svg" href="https://github.com/mattmajestic/go-terraform">
