...
- Before running the `branches` command, ensure that you have [installed the Neon CLI](/reference/cli-install).
- If you have not authenticated with the [neon auth](/reference/cli-auth) command, running a Neon CLI command automatically launches the Neon CLI browser authentication process. Alternatively, you can specify a Neon API key using the `--api-key` option when running a command. See [Connect](/reference/neon-cli#connect).

## The `branches` command

The `branches` command allows you to list, create, rename, delete, and retrieve information about branches in your Neon project. It also permits setting a branch as the default branch, adding a compute to a branch, adding a [read replica](/introduction/read-replicas), or perforning a [schema diff](/guides/schema-diff) between different branches.
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `list` subcommand supports these options:

| Option           | Description                                                                                   | Type   |                      Required                       |
| ---------------- | --------------------------------------------------------------------------------------------- | ------ | :-------------------------------------------------: |
| `--context-file` | [Context file](/reference/cli-set-context#using-a-named-context-file) path and file name | string |                                                     |
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `create` subcommand supports these options:

| Option              | Description                                                                                                                                                                                                                                                                     | Type    |                      Required                       |
| :------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------ | :-------------------------------------------------: |
| `--context-file`    | [Context file](/reference/cli-set-context#using-a-named-context-file) path and file name                                                                                                                                                                                   | string  |                                                     |
| `--project-id`      | Project ID                                                                                                                                                                                                                                                                      | string  | Only if your Neon account has more than one project |
| `--name`            | The branch name. Optional: if not specified, defaults to the branch ID. If provided, must be unique within the project and can be up to 256 characters. Cannot be empty or only whitespace. See [Branch naming requirements](/manage/branches#branch-naming-requirements). | string  |                                                     |
...
| `--type`            | Type of compute to add. Choices are `read_write` (the default) or `read_only`. A read-only compute is a [read replica](/introduction/read-replicas).                                                                                                                       | string  |                                                     |
...
<Note>
When creating a branch from a protected parent branch, role passwords on the child branch are changed. For more information about this Protected Branches feature, see [New passwords generated for Postgres roles on child branches](/guides/protected-branches#new-passwords-generated-for-postgres-roles-on-child-branches).
</Note>
...
- Create a branch with a [read replica](/introduction/read-replicas) compute.
...
- Create an instant restore branch by specifying the `--parent` option with a timestamp:
  ...
  The timestamp must be provided in RFC 3339 format. You can use this [timestamp converter](https://it-tools.tech/date-converter). For more information about instant restore, see [Instant restore](/guides/branch-restore).
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `reset` subcommand supports these options:
...
`<new-name>` is the new name for the branch. It must be unique within the project and can be up to 256 characters. It cannot be empty or only whitespace. See [Branch naming requirements](/manage/branches#branch-naming-requirements) for full details.

#### Options

In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `rename` subcommand supports these options:
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `schema-diff` subcommand supports these options:
...
<Note>
The `--no-color` or `--color false` [global option](/reference/neon-cli#global-options) can be used to decolorize the CLI command output when using CLI commands in CI/CD pipelines.
</Note>
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `set-default` subcommand supports this option:
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `set-expiration` subcommand supports these options:
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `add-compute` subcommand supports these options:

| Option           | Description                                                                                                                                                                                                                                         | Type   |                      Required                       |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | :-------------------------------------------------: |
| `--context-file` | [Context file](/reference/cli-set-context#using-a-named-context-file) path and file name                                                                                                                                                       | string |                                                     |
| `--project-id`   | Project ID                                                                                                                                                                                                                                          | string | Only if your Neon account has more than one project |
| `--type`         | Type of compute to add. Choices are `read_only` (the default) or `read_write`. A read-only compute is a [read replica](/introduction/read-replicas). A branch can have a single primary read-write compute and multiple read replica computes. | string |                                                     |
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `delete` subcommand supports this option:
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `get` subcommand supports this option:
