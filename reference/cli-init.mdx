...
- Before running the `init` command, ensure that you have [installed the Neon CLI](/reference/neon-cli#install-the-neon-cli).
- The `init` command requires authentication. If you have not authenticated with the [neon auth](/reference/cli-auth) command, running `neon init` will automatically launch the Neon CLI browser authentication process. Alternatively, you can specify a Neon API key using the `--api-key` option when running a command. See [Connect](/reference/neon-cli#connect).
...
This command supports [global options](/reference/neon-cli#global-options) only.
...
For detailed manual setup instructions for all editors, see [Connect MCP clients](/ai/connect-mcp-clients-to-neon).
