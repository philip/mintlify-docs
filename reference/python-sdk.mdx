...
Working with AI coding assistants? Check out our [AI rules for the Neon Python SDK](/ai/ai-rules-neon-python-sdk) to help your AI assistant generate better code when managing Neon resources with Python.
</Tip>
...
- `branch_create(project_id, **json)`: Creates a new branch. Branch names are optional (defaults to branch ID if not specified). If provided, names must be unique within the project, can be up to 256 characters, and cannot be empty or only whitespace. See [Branch naming requirements](/manage/branches#branch-naming-requirements).
