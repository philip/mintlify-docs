...
- Before running a `vpc` command, ensure that you have [installed the Neon CLI](/reference/cli-install).
...
You can use the `vpc` CLI command to manage [Private Networking](/guides/neon-private-networking) configurations in Neon.
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `vpc endpoint` subcommand supports these options:
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `vpc project` subcommand supports these options:
