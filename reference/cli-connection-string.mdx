...
- Before running the `connection-string` command, ensure that you have [installed the Neon CLI](/reference/cli-install).
...
In addition to the Neon CLI [global options](/reference/neon-cli#global-options), the `connection-string` command supports these options:
...
- Get a connection string to a specific point in a branch's history by appending `@timestamp` or `@lsn`. Availability depends on your configured [restore window](/introduction/restore-window).

  ```bash
  neon connection-string @2024-04-21T00:00:00Z
  ```

  For additional examples, see [How to use Time Travel](/guides/time-travel-assist#how-to-use-time-travel).
