...
- [**API Keys:**](/manage/api-keys) Create, list, and revoke API keys for secure access to the Neon API.
- [**Projects:**](/manage/projects) Create, list, update, and delete Neon projects.
- [**Branches:**](/manage/branches) Manage branches, including creation, deletion, restoration, and schema management.
- [**Databases:**](/manage/databases) Create, list, update, and delete databases within your branches.
- [**Compute Endpoints:**](/manage/computes) Manage compute endpoints, including creation, scaling, suspension, and restart.
- [**Roles:**](/manage/roles) Create, list, update, and delete Postgres roles within your branches.
- [**Operations:**](/manage/operations) Monitor and track the status of asynchronous operations performed on your Neon resources.
- [**Organizations:**](/manage/orgs-api) Manage organization settings, API keys, and members (for Neon organizational accounts).
- [**Consumption Metrics:**](/guides/consumption-metrics) Retrieve usage metrics for your account and projects to monitor resource consumption.

<Tip title="AI Rules available">
Working with AI coding assistants? Check out our [AI rules for the Neon TypeScript SDK](/ai/ai-rules-neon-typescript-sdk) to help your AI assistant generate better code when managing Neon resources programmatically.
</Tip>
...
- The `region_id` parameter specifies the cloud region where the project will be hosted. You can find the list of supported regions at [Neon Regions](/introduction/regions).
...
- `name` (optional): The branch name. If not provided, defaults to the branch ID. If specified, must be unique within the project and can be up to 256 characters. Cannot be empty or consist only of whitespace. See [Branch naming requirements](/manage/branches#branch-naming-requirements) for details.
...
- Compute Unit (CU) customization (optional): Control compute size using `autoscaling_limit_min_cu` and `autoscaling_limit_max_cu`. Refer to [Compute size and autoscaling configuration](/manage/computes#compute-size-and-autoscaling-configuration) for available options.
...
- Branch & Project IDs: You can obtain these IDs from the [Neon Console](/manage/branches#view-branches) or using SDK methods (for example, [listProjectBranches](#list-branches), [listProjects](#list-projects)).
