...
- **PostgreSQL Anonymizer integration**: Uses the [PostgreSQL Anonymizer extension's](/extensions/postgresql-anonymizer) masking functions
- **Branch-specific rules**: You can define different masking rules for each anonymized Neon branch

<Info title="Static versus dynamic masking">
...
![Neon Console 'Create new branch' dialog with 'Anonymized data' selected](/images/docs/workflows/anon-create-a-new-branch.png)
...
3. You apply the masking rules (in Console, click **Apply masking rules**; via API, call the [start anonymization endpoint](/workflows/data-anonymization-api#start-anonymization)), and the PostgreSQL Anonymizer extension masks the branch data.
...
4. When you are ready, click **Apply masking rules** to start the anonymization job. You can monitor its progress on this page or via the [API](/workflows/data-anonymization-api#get-anonymization-status).

![Neon Console 'data masking' dialog with example masking functions configured](/images/docs/workflows/anon-data-masking.png)
...
4. [Connect](/connect/connect-from-any-app) your development environment to the anonymized branch.
...
- Anonymized branches do not currently support projects with [IP Allow](/manage/projects#configure-ip-allow) or [Private Networking](/guides/neon-private-networking) enabled.
...
- [Data Anonymization API Reference](/workflows/data-anonymization-api) - Complete API documentation with request/response examples
- [Data Anonymization with GitHub Actions](/workflows/data-anonymization-github-actions) - Automate anonymized branch creation in CI/CD
- [PostgreSQL Anonymizer documentation](https://postgresql-anonymizer.readthedocs.io/)
- [Neon branching overview](/introduction/branching)
